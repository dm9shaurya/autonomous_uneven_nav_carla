...
class VSGPSubgoalPlanner:
    ...
    def plan(
        ...
        if not candidates:
            print("[SUBGOAL] No constrained candidates – using relaxed fallback")
            dist_veh = math.hypot(goal_pos[0] - vehicle_pos[0], goal_pos[1] - vehicle_pos[1])
            for a in alpha_range:
                d = self.min_dist + 2.0
                lx = d * math.cos(a)
                ...
                sg = Subgoal(
                    ...
                    local_pos=np.array([lx, ly, 0.0], dtype=np.float32),
                    world_pos=np.array([wx, wy, 0.0], dtype=np.float32),
                    slope=0.5, roughness=0.5, occupancy=0.5,
                    variance=0.5, traversability=0.3,
                    terrain_cost=5.0, goal_progress=goal_progress,
                    heading_error=heading_err, safe=False,
                    width_m=self.cfg.wheel_base, cost=cost,
                )
                candidates.append(sg)
...
# ╔════════════════════════════════════════════════════════════════════════════╗
# §8  STUCK MEMORY  (feeds MPPI cost net; no recovery logic)
# ╚════════════════════════════════════════════════════════════════════════════╝
...
class StuckMemory:
    ...
    def update(self, pos: np.ndarray) -> None:
        """Decay existing points; no stuck-marking logic."""
        self.points = [
            (x, y, w * self.decay, t)
...
# ╔════════════════════════════════════════════════════════════════════════════╗
# §17.6  BEHAVIOR PLANNER  (no recovery mode)
# ╚════════════════════════════════════════════════════════════════════════════╝
...
# ╔════════════════════════════════════════════════════════════════════════════╗
# §17.8  CONTROL ARBITRATOR  (no recovery branch)
# ╚════════════════════════════════════════════════════════════════════════════╝
...
class ControlArbitrator:
    ...
    def arbitrate(
        ...
        # AUTO: apply semantic / IMU speed cap if needed
        cmd = reactive_cmd
...
class NavigationSystem:
    def __init__(self, cfg: Config = CFG):
        ...
        pygame.display.set_caption(
            "VSGP-MPPI Off-Road Navigator  |  TAB=AUTO/MANUAL  ESC=quit"
        )
    ...
    def _tick(self):
        ...
        else:  # AUTO / AVOID_OBSTACLE / GOAL_REACHED
            if perc is not None:
