...
class Config:
    ...
    fleet_size: int = 5
    formation_radius: float = 8.5
    formation_lookahead_m: float = 8.0
    formation_heading_kp: float = 1.65
    formation_lateral_kp: float = 0.18
    formation_speed_cap: float = 8.5
    formation_brake_gain: float = 0.35
    formation_max_steer: float = 0.95
    ...
    def _load_best_route(self) -> None:
        ...
        try:
            rows = self._conn.execute(
                ...
                SELECT run_id,
                       COUNT(*) AS n_samples,
                       MAX(success) AS success_hits,
                       MIN(goal_dist) AS min_goal_dist,
                       MAX(goal_progress) AS max_progress,
                       AVG(traversability) AS avg_traversability,
                       AVG(terrain_cost) AS avg_terrain_cost,
                       SUM(CASE WHEN traversability >= 0.60 THEN 1 ELSE 0 END) * 1.0 / COUNT(*) AS safe_ratio
                FROM samples
                GROUP BY run_id
        ...
        scored: List[Tuple[float, float, float, float, int, np.ndarray]] = []
        fallback: List[Tuple[float, float, float, float, int, np.ndarray]] = []
        ...
        for r in rows:
            ...
            avg_traversability = float(r[5]) if r[5] is not None else 0.0
            avg_terrain_cost = float(r[6]) if r[6] is not None else 1.0
            safe_ratio = float(r[7]) if r[7] is not None else 0.0
            ...
            # Prefer long, smooth, safe routes; shorter routes only survive if they are
            # much safer and more successful than the alternatives.
            score = (
                length * (0.50 + 1.20 * avg_traversability + 0.80 * safe_ratio)
                + 20.0 * max_progress
                - 0.5 * min_goal_dist
                - 8.0 * avg_terrain_cost
            )
            entry = (score, length, safe_ratio, avg_traversability, run_id, pts)
        ...
        # Keep only the single best path: the rest stay in the database, but they do not
        # influence current route guidance.
        chosen.sort(key=lambda item: (item[0], item[1], item[2], item[3]), reverse=True)
        score, length, safe_ratio, avg_trav, run_id, pts = chosen[0]
        self._route_run_id = int(run_id)
        self._route_points = pts.astype(np.float32)
        self._route_length = float(length)
        self._route_score = float(score)
        self._route_safe_ratio = float(safe_ratio)
        self._route_avg_traversability = float(avg_trav)
        if len(pts) >= 2:
            seg = np.linalg.norm(np.diff(pts[:, :2], axis=0), axis=1)
...
class FleetTerrainNavigationSystem(TerrainAwareNavigationSystem):
    """Leader-based fleet controller with circular formation followers."""

    def __init__(self, cfg: Config = CFG):
        super().__init__(cfg)
        self._fleet_followers: List = []
        self._fleet_size = max(1, int(getattr(cfg, "fleet_size", 5)))
        self._formation_radius = float(getattr(cfg, "formation_radius", 8.5))
        self._formation_lookahead_m = float(getattr(cfg, "formation_lookahead_m", 8.0))
        self._formation_heading_kp = float(getattr(cfg, "formation_heading_kp", 1.65))
        self._formation_lateral_kp = float(getattr(cfg, "formation_lateral_kp", 0.18))
        self._formation_speed_cap = float(getattr(cfg, "formation_speed_cap", 8.5))
        self._formation_brake_gain = float(getattr(cfg, "formation_brake_gain", 0.35))
        self._formation_max_steer = float(getattr(cfg, "formation_max_steer", 0.95))
        self._fleet_last_controls: Dict[int, ControlState] = {}

    def spawn_vehicle(self):
        super().spawn_vehicle()
        self._spawn_fleet_followers()

    def _spawn_fleet_followers(self):
        if self._fleet_size <= 1 or self._vehicle is None:
            return
        bp_lib = self._world.get_blueprint_library()
        bp = bp_lib.find(self.cfg.vehicle_blueprint)
        if bp.has_attribute("role_name"):
            bp.set_attribute("role_name", "fleet_follower")

        leader_tf = self._vehicle.get_transform()
        leader_yaw = math.radians(float(leader_tf.rotation.yaw))
        cx = float(leader_tf.location.x)
        cy = float(leader_tf.location.y)
        cz = float(leader_tf.location.z)

        self._fleet_followers.clear()
        needed = max(0, self._fleet_size - 1)
        for i in range(needed):
            angle = 2.0 * math.pi * float(i + 1) / float(self._fleet_size)
            spawned = None
            for dz in (0.0, 0.4, 0.8, 1.2, 1.8):
                px = cx + self._formation_radius * math.cos(leader_yaw + angle)
                py = cy + self._formation_radius * math.sin(leader_yaw + angle)
                tf = carla.Transform(
                    carla.Location(x=px, y=py, z=cz + dz),
                    carla.Rotation(pitch=leader_tf.rotation.pitch, yaw=leader_tf.rotation.yaw, roll=0.0),
                )
                try:
                    spawned = self._world.try_spawn_actor(bp, tf)
                except RuntimeError:
                    spawned = None
                if spawned is not None:
                    break
            if spawned is None:
                print(f"[FLEET] follower {i+1} spawn failed")
                continue
            spawned.set_simulate_physics(True)
            spawned.apply_control(carla.VehicleControl(throttle=0.0, brake=1.0))
            self._fleet_followers.append(spawned)
            print(f"[FLEET] follower {i+1} spawned")

        if self._fleet_followers:
            self._safe_tick(int(self.cfg.post_spawn_wait_s / self.cfg.fixed_delta_seconds))
            for v in self._fleet_followers:
                try:
                    v.apply_control(carla.VehicleControl(throttle=0.0, brake=0.0))
                except Exception:
                    pass

    def _formation_center_and_heading(self):
        leader_tf = self._vehicle.get_transform()
        leader_xy = np.array([leader_tf.location.x, leader_tf.location.y], dtype=np.float32)
        heading = math.radians(float(leader_tf.rotation.yaw))
        center = leader_xy.copy()

        rg = self._route_guidance
        route_pts = getattr(self._persistent_memory, "_route_points", None)
        if rg is not None and rg.active and rg.target_xy is not None:
            center = np.array(rg.target_xy[:2], dtype=np.float32)
            if route_pts is not None and rg.target_idx >= 0 and len(route_pts) > 1:
                i = int(np.clip(rg.target_idx, 0, len(route_pts) - 2))
                j = i + 1
                vec = route_pts[j, :2] - route_pts[i, :2]
                if float(np.linalg.norm(vec)) > 1e-6:
                    heading = math.atan2(float(vec[1]), float(vec[0]))
            else:
                vec = center - leader_xy
                if float(np.linalg.norm(vec)) > 1e-6:
                    heading = math.atan2(float(vec[1]), float(vec[0]))
        return center, heading

    def _vehicle_state_from_actor(self, veh) -> VehicleState:
    ...
    def _control_to_target(self, veh, target_xy: np.ndarray, heading_ref: float, slot_idx: int) -> ControlState:
        st = self._vehicle_state_from_actor(veh)
        dx = float(target_xy[0] - st.x)
        dy = float(target_xy[1] - st.y)
        dist = math.hypot(dx, dy)
        target_yaw = math.atan2(dy, dx)
        err = angle_wrap(target_yaw - st.yaw)

        # Cross-track pull helps the ring stay circular instead of spiraling out.
        left = np.array([-math.sin(heading_ref), math.cos(heading_ref)], dtype=np.float32)
        crosstrack = float(np.dot(np.array([dx, dy], dtype=np.float32), left))

        steer = (
            self._formation_heading_kp * err
            + self._formation_lateral_kp * crosstrack / max(self._formation_radius, 1e-3)
        )
        steer = clamp(steer, -self._formation_max_steer, self._formation_max_steer)

        speed_cap = self._formation_speed_cap
        if self._route_guidance is not None:
            speed_cap *= clamp(1.15 - 0.18 * self._route_guidance.corridor_distance / max(self.cfg.persistent_route_corridor_radius_m, 1.0), 0.45, 1.0)

        # Harder cornering at low speed, softer when already stable.
        target_speed = clamp(1.2 + 1.4 * dist, 1.5, speed_cap)
        throttle = clamp((target_speed - st.speed) / max(target_speed, 1e-3), 0.0, self.cfg.max_throttle)
        brake = 0.0
        if st.speed > target_speed * 1.08:
            brake = clamp((st.speed - target_speed) * self._formation_brake_gain, 0.0, self.cfg.max_brake)
            throttle = 0.0
        ...
        return ControlState(throttle=throttle, steer=steer, brake=brake, reverse=False)

    def _update_fleet_followers(self):
        if not self._fleet_followers or self._vehicle is None:
            return
        if not self._vehicle.is_alive:
            return

        center, heading = self._formation_center_and_heading()
        n = len(self._fleet_followers)
        for idx, veh in enumerate(self._fleet_followers):
            if veh is None or not veh.is_alive:
                continue
            ang = heading + 2.0 * math.pi * float(idx + 1) / float(max(n + 1, 2))
            target_xy = np.array([
                center[0] + self._formation_radius * math.cos(ang),
                center[1] + self._formation_radius * math.sin(ang),
            ], dtype=np.float32)
            cmd = self._control_to_target(veh, target_xy, heading, idx)

            # Shared terrain/route prior: slow down when the leader's route is uncertain.
            if self._route_guidance is not None and self._route_guidance.active:
                route_pen = clamp(self._route_guidance.corridor_distance / max(self.cfg.persistent_route_corridor_radius_m, 1.0), 0.0, 2.0)
                cmd.throttle *= clamp(1.0 - 0.20 * route_pen, 0.45, 1.0)
                if route_pen > 1.0:
                    cmd.brake = max(cmd.brake, clamp(0.15 * route_pen, 0.0, self.cfg.max_brake))

            # Apply the same safety style: sharper steering, but no reckless acceleration.
            veh.apply_control(carla.VehicleControl(
                throttle=clamp(cmd.throttle, 0.0, 1.0),
                steer=clamp(cmd.steer, -1.0, 1.0),
                brake=clamp(cmd.brake, 0.0, 1.0),
                reverse=False,
                hand_brake=False,
            ))
            self._fleet_last_controls[id(veh)] = cmd

    def _fleet_stop(self):
        for veh in self._fleet_followers:
            try:
                if veh is not None and veh.is_alive:
                    veh.apply_control(carla.VehicleControl(throttle=0.0, steer=0.0, brake=1.0, reverse=False, hand_brake=False))
            except Exception:
                pass
    ...
    def _tick(self):
        ...
        if not self._running:
            self._fleet_stop()
            return
        self._update_fleet_followers()

# Rebind the public names so the entry point uses the enhanced versions.
NavigationSystem = FleetTerrainNavigationSystem
