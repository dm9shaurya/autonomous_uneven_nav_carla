...
class Config:
    ...
    subgoal_distance: float = 16.0
    subgoal_min_distance: float = 3.0
    subgoal_num_angles: int = 16
    subgoal_num_depth: int = 18
    ...
    # ── Reactive Avoidance
    react_warn_dist: float = 12.0
    react_danger_dist: float = 7.5
    react_emergency_dist: float = 3.2
    react_forward_cone: float = 60.0

    # ── Control Limits
    max_throttle: float = 0.98
    max_steer: float = 0.88
    max_brake: float = 0.65
    min_throttle_auto: float = 0.70
    min_throttle_hill: float = 0.82
    target_speed_flat: float = 13.0
    target_speed_bad: float = 6.2
    target_speed_near_goal: float = 3.2
    kp_speed: float = 1.45
    throttle_rate: float = 0.22
    ...
    speed_steer_coupling: float = 0.42
    min_steer_at_speed: float = 0.10

    # ── Terrain Speed Caps
    max_speed_on_slope: float = 5.4
    max_speed_on_rough: float = 5.6
    ...
    curvature_weight: float = 3.8
    void_weight: float = 6.5
...
class TopologicalMemory:
    ...
    def corridor_bias(self, wx, wy) -> float:
        ...
        for n in self.nodes:
            ...
            if n.is_safe:
                best_safe = max(best_safe, prox * (1.0 - n.cost))
            else:
...
class VSGPSubgoalPlanner:
    CURV_SAFE_THRESH  = 0.82
    VOID_SAFE_THRESH  = 0.40
    OCC_FREE_THRESH   = 0.58
    VAR_STABLE_THRESH = 0.68
    SLOPE_SAFE_THRESH = 0.72
    ROUGH_SAFE_THRESH = 0.72
    ...
    SEGMENT_STEP_M = 1.5
    SEGMENT_MIN_TRAV = 0.52
    SEGMENT_MIN_CLEARANCE = 0.78
    SEGMENT_MAX_COST = 0.74
    GOAL_FOCUS_BONUS = 1.8
    ...
    def plan(self, perc, vehicle_pos, vehicle_yaw_rad, goal_pos,
        ...
        dyn_max_dist  = min(self.max_dist, max(self.min_dist + 2.0, sem_speed_cap * 1.6))
        ...
        for a in alpha_range:
            ...
            for d in depths:
                ...
                safe_here = (
                    o_v < self.OCC_FREE_THRESH and v_v < self.VAR_STABLE_THRESH
            ...
            mem_cost = lookahead_c = topo_cost = corridor_cost = 0.0
            if terrain_memory:
            ...
            if tcn_nav_world is not None:
                corridor_dist = float(np.linalg.norm(np.array([wx, wy], dtype=np.float32) - tcn_nav_world[:2]))
                corridor_cost = corridor_dist / max(tcn_corridor_bias, 1e-6)

            cost = (
                terrain_cost
                + self.W_GOAL_DIST * dist_c
                + self.W_HEADING * heading_err
                + 0.65 * mem_cost
                + lookahead_c + sem_cost + topo_cost + corridor_cost
                - self.GOAL_FOCUS_BONUS * progress
            )
            ...
            sg = Subgoal(
                ...
                width_m=max(2.0, self.cfg.wheel_base * 1.25),
                cost=cost,
            )
            candidates.append(sg)

        if not candidates:
            dist = min(25.0, max(self.min_dist, math.hypot(dx_world, dy_world)))
            lx = dist * math.cos(goal_rel_y)
            ly = dist * math.sin(goal_rel_y)
            wx, wy = local_to_world(lx, ly, vehicle_yaw_rad, vehicle_pos[0], vehicle_pos[1])
            slope_, rough_, obs, flat, clear = self._fallback_risk(perc, lx, ly)
            return Subgoal(
                alpha=goal_rel_y, beta=0.0, distance=dist,
                local_pos=np.array([lx, ly, 0.0], dtype=np.float32),
                world_pos=np.array([wx, wy, 0.0], dtype=np.float32),
                slope=slope_, roughness=rough_, occupancy=obs, variance=0.25,
                traversability=flat, terrain_cost=slope_ + rough_ + obs,
                goal_progress=0.0, heading_error=abs(goal_rel_y), safe=False,
                width_m=max(2.0, self.cfg.wheel_base * 1.25), cost=math.hypot(dx_world, dy_world),
            ), []

        candidates.sort(key=lambda c: (c.cost, c.goal_dist, c.heading_error))
        best = candidates[0]
        close = [c for c in candidates if c.cost <= best.cost + 8.0]
        if close:
            close.sort(key=lambda c: (-c.progress, c.goal_dist, c.heading_error, c.cost))
            best = close[0]
        self._last_best = best
        return best, candidates

    def _fallback_risk(self, perc, lx: float, ly: float) -> Tuple[float, float, float, float, float]:
        idx_a = int(np.argmin(np.abs(perc.alpha_grid[0] - math.atan2(ly, lx))))
        mid_b = perc.traversability.shape[0] // 2
        slope = float(perc.slope_map[mid_b, idx_a])
        rough = float(perc.roughness_map[mid_b, idx_a])
        obs   = float(perc.occupancy_mean[mid_b, idx_a])
        flat  = float(perc.traversability[mid_b, idx_a])
        clear = float(perc.void_risk_map[mid_b, idx_a])
        return slope, rough, obs, flat, clear
...
class ReactiveObstacleAvoidance:
    ...
    def process(self, raw_pts, ctrl, speed):
        ...
        fwd_mask = (
            ...
            & (z > -0.4) & (dist_2d < self.cfg.react_warn_dist * 1.6)
        )
        ...
        steer_dir = -np.sign(obs_y) if abs(obs_y) > 0.08 else -np.sign(ctrl.steer + 1e-6)

        if min_dist <= self.cfg.react_emergency_dist:
            out.throttle = 0.12
            out.brake = self.cfg.max_brake
            out.reverse = True
            out.steer = float(np.clip(ctrl.steer + steer_dir * self.cfg.max_steer,
            ...
            ratio = (min_dist - self.cfg.react_emergency_dist) / (
                self.cfg.react_danger_dist - self.cfg.react_emergency_dist + 1e-6)
            steer_mag = min(self.cfg.max_steer, 0.90 / max(min_dist, 0.35))
            out.throttle = max(0.08, ctrl.throttle * max(0.15, ratio * 0.6))
            out.brake = self.cfg.max_brake * (1.0 - ratio * 0.55)
            out.steer = float(np.clip(ctrl.steer + steer_dir * steer_mag,
                                      -self.cfg.max_steer, self.cfg.max_steer))
        else:
            ratio = (min_dist - self.cfg.react_danger_dist) / (
                self.cfg.react_warn_dist - self.cfg.react_danger_dist + 1e-6)
            steer_mag = min(self.cfg.max_steer * 0.85, 0.55 / max(min_dist, 0.5))
            out.throttle = max(ctrl.throttle, 0.25 + 0.35 * ratio)
            out.brake = max(0.0, self.cfg.max_brake * (1.0 - ratio * 0.8))
            out.steer = float(np.clip(ctrl.steer + steer_dir * steer_mag,
                                      -self.cfg.max_steer, self.cfg.max_steer))
        return out
...
class Controller:
    ...
    def apply_safety_filters(self, ctrl, roll_deg, pitch_deg, speed=0.0):
        # Less smoothing -> quicker steering response, especially on low-speed off-road turns.
        a = 0.45
        ...
        st = float(np.clip(st, self._state.steer    - 0.20, self._state.steer    + 0.20))
        th = float(np.clip(th, self._state.throttle - 0.12, self._state.throttle + 0.12))
        ts, cs = self._stability(roll_deg, pitch_deg)
        ts = max(0.25, ts)
        st = float(np.clip(st + cs * 1.25, -max_steer_at_speed, max_steer_at_speed))
        st -= 0.12 * (st - self._state.steer)
        th -= 0.08 * (th - self._state.throttle)
...
class ControlArbitrator:
    ...
    def arbitrate(self, behavior_mode, safety_unsafe, reactive_cmd, mppi_cmd,
        ...
        # Goal-hungry: keep creeping forward unless we are actually done or safety says stop.
        if goal_dist > self.cfg.goal_tolerance and not cmd.reverse:
            min_thr = self.cfg.min_throttle_hill if (abs(st.pitch_deg) > 7.0 or slope > 0.38 or roughness > 0.45) else self.cfg.min_throttle_auto
            if obstacle_risk < 0.80:
                cmd.throttle = max(cmd.throttle, min_thr)
            if cmd.brake < 0.90:
                cmd.brake = min(cmd.brake, 0.12)
        return cmd, "AUTO"
...
class NavigationSystem:
    ...
    def _apply_throttle_bias(self, ctrl, st, goal_dist, slope, roughness,
        ...
        bad = clamp(0.70 * slope + 0.45 * roughness + 0.30 * obstacle_risk, 0.0, 1.0)
        ...
        target_speed *= max(0.55, 1.0 - 0.25 * abs(out.steer) / max(self.cfg.max_steer, 1e-6))
        if semantic is not None or imu is not None:
        ...
        min_thr = (self.cfg.min_throttle_hill
                   if (abs(st.pitch_deg) > 7.0 or slope > 0.38 or roughness > 0.45)
