...
class MPPIPlanner:
    def __init__(self, cfg: Config):
        ...
        self._cost_buf = torch.zeros((self.num_samples,), device=self.device)
        
        # Grid Cache (Persistent GPU tensors)
        self._gpu_occ = torch.zeros((cfg.vsgp_n_inducing, cfg.vsgp_n_inducing), device=self.device)
        ...
        self._grid_shape = (PerceptionModule.BETA_RES, PerceptionModule.ALPHA_RES)
    ...
    def plan(self, state, perc: PerceptionOutput) -> ControlState:
        ...
        # 5. Rollout & Cost
        costs = self._rollout_cost(state, u_samples, occ_t, slope_t, rough_t, var_t)
...
class Controller:
    ...
    def apply_safety_filters(
        ...
        out_steer = float(np.clip(out_steer + corrective_steer, -self.cfg.max_steer, self.cfg.max_steer))
...
class VisualizationModule:
    ...
    def _draw_velocity(self, ax):
        ...
        ax.plot(xs, list(self._vel_ang), c="#a29bfe", lw=1.0, label="omega*v (m/s)")
        ax.legend(fontsize=6, loc="upper left",
                  facecolor="#222", edgecolor="#555", labelcolor="#ddd")
        ax.set_xlabel("Step", fontsize=7)
...
class ManualController:
    ...
    def tick(self, keys) -> ControlState:
        ...
        return self.ctrl.apply_safety_filters(
            ControlState(throttle=vx / CFG.base_speed, steer=self._steer), 0, 0
        )
