...
# FIX 1: Removed duplicate erroneous `from turtle import st` lines (×2).
#         `st` is not a turtle export and was never used anywhere in the code.
...
class Config:
    ...
    anti_tip_roll_danger: float = 30.0
    anti_tip_pitch_warn: float = 30.0
    anti_tip_pitch_danger: float = 50.0
...
class PerceptionModule:
    ...
    def process_lidar(self, measurement) -> Optional[PerceptionOutput]:
        ...
        # --- Void / cliff risk from forward LiDAR sparsity + discontinuity ---
        ...
        # FIX 2: `np.funp.meanike` was a garbled typo; corrected to `np.full_like`.
        # FIX 3: Removed the duplicate slope/roughness/_norm/slope_n/rough_n/occ_n/var_n
        #         block that followed this line in the original (it re-computed everything
        #         already computed above, overwriting valid values with identical ones).
...
class VSGPSubgoalPlanner:
    ...
    def plan(
        ...
        # FIX 4: `trav` was used before assignment, causing NameError.
        #         Use `perc.traversability` directly as the shape reference.
...
class MPPIPlanner:
    ...
    def plan(
        ...
        # FIX 5: The original code called _rollout TWICE — the first call used the
        #         wrong (old) signature missing `curv` and `void_map`, computed weights
        #         from those wrong costs, then a second (correct) call result was simply
        #         discarded.  Now there is a single correct call; weights come from it.
    ...
    def _rollout(self, state, u, occ, slope, rough, var, trav, curv, void_map):
        ...
        for t in range(T):
            ...
            # Lateral G-force proxy – penalise sharp steer at speed
            lateral_g = (steer.abs() * v.abs()) / max(self.cfg.wheel_base, 1e-3)
            cost += 12.0 * lateral_g.clamp(0.0, 3.0) ** 2

            # Speed-terrain risk – penalise high speed on bad terrain
...
class NavigationSystem:
    ...
    def _anti_tip_governor(
        ...
        # 2. Pitch: graduated throttle cut
    ...
    def _tick(self) -> None:
        ...
        if self._mode == "MANUAL":
            ...
            if rec_cmd is not None:
                ...
                # FIX 6: Guard against best_subgoal being None before accessing .local_pos.
                # FIX 7: Removed erroneous `best_subgoal.clearance_risk` (field doesn't exist
                #         in Subgoal); use the `clearance` variable from terrain.local_risk().
                clearance = 0.0
                ...
                if best_subgoal is None:
                    ...
                    current_clearance_risk = clearance   # from terrain.local_risk() above
        ...
        if all_candidates and all_candidates[0] is not None:
            ...
            # FIX 8: Corrected positional field ordering in the fallback Candidate().
            #         Previously slope/roughness/obstacle_risk/cost/reward were all wrong.
            cand = Candidate(
                st.x, st.y, 0, 0, 0, 0,
