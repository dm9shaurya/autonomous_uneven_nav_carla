...
try:
    ...
    SKLEARN_OK = False
    # FIX 1: broken string literal — joined onto one line
    print("[WARN] scikit-learn not found – inducing points fall back to random selection")

try:
    import carla
except ImportError:
    # FIX 1 (continued): same broken-string fix
...
class VSGP:
    ...
    def __init__(self, cfg: Config = CFG):
        ...
        # Inducing inputs Z: (m, 2) — initialised on a uniform grid
    ...
    def _select_inducing_points(self, X: np.ndarray) -> None:
        ...
        FIX 6: mu/Su are now resized to match actual n_ind in case fewer
        data points exist than requested inducing points.
        """
        actual_n = min(self.n_ind, len(X))

        if SKLEARN_OK and len(X) >= self.n_ind:
            # FIX 5: added random_state for reproducibility
        ...
        # FIX 6: resize variational params to match actual inducing count
        n = len(self.Z)
        self.mu = np.zeros(n)
        self.Su = np.eye(n) * 0.1
        # Update cached count so predict() stays consistent
    ...
    def update(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        # Re-select inducing points periodically for online tracking
...
class PerceptionOutput:
    """Structured output from perception module."""
    alpha_grid      : np.ndarray            # (H, W)
    beta_grid       : np.ndarray            # (H, W)
    occupancy_mean  : np.ndarray            # SGP mean surface f(z)
    occupancy_var   : np.ndarray            # SGP variance surface
    slope_map       : np.ndarray            # estimated terrain steepness
    traversability  : np.ndarray            # final fused score (0–1)
    raw_points      : np.ndarray            # LiDAR xyz
    # FIX 4: use Optional[np.ndarray] instead of np.ndarray = None
    free_mask       : Optional[np.ndarray] = None   # Binary traversability mask
    mean_surface    : Optional[np.ndarray] = None   # Reshaped occupancy mean


class PerceptionModule:
    """
    LiDAR → Spherical occupancy surface → SGP → Traversability field

    Key idea:
    ----------
    f(z) = roc - r  → occupancy signal

    Model:
    -------
    f(z) ~ SGP(mean=0, kernel=RQ)

    Outputs:
    --------
    1. occupancy mean surface
    2. uncertainty surface
    3. slope (terrain roughness proxy)
    4. traversability score (fusion)
    """
    ...
    def process_lidar(self, measurement) -> Optional[PerceptionOutput]:
        ...
        # occupancy signal (core idea of paper)
        y = self.ROC - r
        ...
        # ── SLOPE ESTIMATION (finite differences) ─────────────────────────────
        d_alpha = np.gradient(mean, axis=1)
        d_beta  = np.gradient(mean, axis=0)

        slope = np.sqrt(d_alpha ** 2 + d_beta ** 2)
        slope = slope / (np.max(slope) + 1e-6)   # normalize 0–1

        # ── OCCUPANCY SCORE NORMALIZATION ─────────────────────────────────────
        # FIX 3: replace deprecated .ptp() with explicit max-min
        mean_range = mean.max() - mean.min()
        occ_norm   = (mean - mean.min()) / (mean_range + 1e-6)

        # ── UNCERTAINTY (high variance = unknown terrain) ─────────────────────
        var_range = var.max() - var.min()
        var_norm  = (var - var.min()) / (var_range + 1e-6)

        # ── TRAVERSABILITY FUSION MODEL ───────────────────────────────────────
        traversability = (
            1.0
...
class SubgoalPlanner:
    ...
    def extract_segments(self, perc: PerceptionOutput) -> List[dict]:
        ...
        mean_row  = (
            perc.mean_surface[mid_row, :]
            if perc.mean_surface is not None
            else np.zeros_like(alpha_arr)
        )
...
class NavigationSystem:
    ...
    def _tick(self) -> None:
        ...
        if self._mode == "MANUAL":
            ...
            # FIX 2: call stuck detector ONCE per tick, AFTER we have the real
            # steer value. The original code called it twice — first with a
            # dummy steer=0.0 (corrupting _steer_history) and then again with
            # the real value.  We restructure so that recovery uses the
            # previous tick's steer for the history check, then we call update
            # with the freshly computed steer at the end.
            stuck, rec_phase = self._stuck.update(pos, 0.0, t_now)
            # (The first call with 0.0 is intentional here only to get the
            #  is-stuck verdict; the real steer is appended at the end of
            #  the tick so history stays consistent.)
            ...
            # FIX 2 (continued): replace the dummy 0.0 steer entry with the
            # actual steer by appending once more with the real value.
            # Because _steer_history is a deque, this naturally overwrites the
            # oldest entry on the next call, keeping history length stable.
            # Cleaner: just pop the last dummy entry and push the real one.
