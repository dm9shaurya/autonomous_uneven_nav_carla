...
from trial.aut1 import ControlState
...
dtype=np.float32).reshape(-1, 4)[:, :3]
        ...
        var_norm = (np.var(var, axis=None) - var.min()) / (var_range + 1e-6) if var_range > 0 else var
...
device=self.device)
        self._noise_buf = torch.zeros((self.num_samples, self.horizon, 2), 
device=self.device)
        self._cost_buf = torch.zeros((self.num_samples,), 
device=self.device)
        
        # Grid Cache (Persistent GPU tensors)
        self._gpu_occ = torch.zeros((cfg.vsgp_n_inducing, 
cfg.vsgp_n_inducing), device=self.device)
        self._gpu_slope = torch.zeros_like(self._gpu_occ)
        self._gpu_rough = torch.zeros_like(self._gpu_occ)
        self._gpu_var = torch.zeros_like(self._gpu_occ)
        self._grid_shape = (PerceptionModule.BETA_RES, 
PerceptionModule.ALPHA_RES)

    def plan(self, state, perc: PerceptionOutput) -> ControlState:
        ...
        # 1. Prepare Nominal Control (shifted from previous step)
        ...
        # 3. Clip Controls
        u_samples[:, :, 0] = torch.clamp(u_samples[:, :, 0], 0.0, 
self.cfg.max_throttle)
        u_samples[:, :, 1] = torch.clamp(u_samples[:, :, 1], 
-self.cfg.max_steer, self.cfg.max_steer)
        
        # 4. Update GPU Maps (Copy from CPU numpy)
        # Only copy if shapes match, else resize
        occ_t = torch.tensor(perc.occupancy_mean, device=device).float()
        slope_t = torch.tensor(perc.slope_map, device=device).float()
        rough_t = torch.tensor(perc.roughness_map, device=device).float()
        var_t = torch.tensor(perc.occupancy_var, device=device).float()
        
        # 5. Rollout & Cost
        costs = self._rollout_cost(state, u_samples, occ_t, slope_t, 
rough_t, var_t)
...
device=device).float().unsqueeze(0).repeat(K, 1)
        ...
        for t in range(T):
            ...
            v = x[:, 3] + throttle * self.dt
            # Prevent division by zero in tan
            steer_clipped = torch.clamp(steer, -0.99, 0.99)
            yaw_rate = (v / self.cfg.wheel_base) * 
torch.tan(steer_clipped) 
        yaw = x[:, 2] + yaw_rate * self.dt
            ...
            # Map Beta (Pitch) - Simplified: assume flat beta for now or map by distance
            # To match PerceptionModule, beta is elevation angle. 
            # Without height rollout, we assume beta=0 (middle index)
...
# ╔════════════════════════════════════════════════════════════════════════╔════════════════════════════════════════════════════════════════════════╗
# §5  CONTROLLER MODULE (Safety & Smoothing)
# ╚════════════════════════════════════════════════════════════════════════╚════════════════════════════════════════════════════════════════════════╝
...
ControlState:
        ...
        # Stability Modifiers
        throttle_scale, corrective_steer = 
self._stability_modifiers(roll_deg, pitch_deg)
...
self.cfg.max_steer))
        ...
        # Final Clip
        out_steer = float(np.clip(out_steer, -self.cfg.max_steer, 
self.cfg.max_steer))
...
self.cfg.max_throttle))
    ...
    def _stability_modifiers(self, roll_deg: float, pitch_deg: float) -> 
Tuple[float, float]:
        throttle_scale = 1.0
...
class StuckDetector:
    ...
    def update(self, pos: np.ndarray, steer: float, t: float) -> 
Tuple[bool, int]:
    ...
    def get_recovery_control(self, phase: int) -> ControlState:
        if phase == 0:
            return ControlState(throttle=0.3, steer=0.0,  brake=0.0, 
reverse=True)
        elif phase == 1:
            return ControlState(throttle=0.3, steer=0.4,  brake=0.0, 
reverse=True)
        else:
            return ControlState(throttle=0.3, steer=-0.4, brake=0.0, 
reverse=True)
...
daemon=True)
    ...
    def _draw_cost(self, ax):
        ...
        if n == 0:
            return
        xs = np.arange(n)
        ax.plot(xs, list(self._cost_total), c="#ff6b6b", lw=1.2, 
label="Total Cost")
    ...
    def _draw_velocity(self, ax):
        ...
        if n == 0:
            return
        xs = np.arange(n)
        ax.plot(xs, list(self._vel_lin), c="#55efc4", lw=1.2, 
label="Linear (m/s)")
...
class ManualController:
    ...
    def tick(self, keys) -> ControlState:
        ...
        if keys[pygame.K_SPACE]:
            self._steer = 0.0
        return 
self.ctrl.apply_safety_filters(ControlState(throttle=vx/CFG.base_speed, 
steer=self._steer), 0, 0)
...
class NavigationSystem:
    ...
    def connect(self) -> None:
        print(f"[CARLA] Connecting to 
{self.cfg.carla_host}:{self.cfg.carla_port} …")
...
self.cfg.carla_port)
    ...
    def setup(self) -> None:
        self._vehicle, self._spawn_z = spawn_vehicle(self._world, 
self.cfg)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_TAB:
                    self._mode = "MANUAL" if self._mode == "AUTO" else 
"AUTO"
...
────────────────────────────────────────────
            stuck, rec_phase = self._stuck.update(pos, ctrl_state.steer, 
t_now)
            ...
            if stuck:
                ...
                if perc is not None:
                    ...
                    # MPPI returns raw controls
                    ctrl_state = self._planner.plan(state, perc)
                else:
                    # Fallback: creep straight forward
                    ctrl_state = ControlState(throttle=0.1, steer=0.0, 
brake=0.0, reverse=False)
                    print("[NAV] Fallback: creeping forward")

        # ── Apply Safety Filters (Rate limiting, Stability)
        # Even MPPI outputs need safety smoothing
