...
class Config:
    ...
    # Setting this True swaps reverse flag and negates steer at 
    ...
    # ── Vehicle Terrain Limits (NEW - Cybertruck specific)
    ground_clearance: float = 0.406   # Tesla Cybertruck ≈ 16 inches
    wheel_base: float = 3.807         # real spec (meters)
    track_width: float = 1.92
    max_step_height: float      = 0.30           # max obstacle height climbable
    ...
    # ── Control penalty weights (NEW - for MPC-like smoothness)
...
# ╔═══════════════════════════════════════════════════════════════════════════╗
# §0b  ENVIRONMENT COST FUNCTION HELPER  (NEW - for MPC/future use)
# ╚═══════════════════════════════════════════════════════════════════════════╝

def compute_env_cost(
    mean: np.ndarray,
    var: np.ndarray,
    slope: np.ndarray,
    ...
    """
    Compute environment cost map combining occupancy, uncertainty,
    terrain slope, and roughness.

    Higher cost = less traversable

    Returns:
        cost_map: np.ndarray of same shape as inputs
    """
    # Normalize each component to [0, 1] range
    mean_norm = (mean - mean.min()) / (mean.max() - mean.min() + 1e-6)
    var_norm = (var - var.min()) / (var.max() - var.min() + 1e-6)
...
# ╔═══════════════════════════════════════════════════════════════════════════╗
# §3  PERCEPTION MODULE (MODIFIED - added roughness calculation)
# ╚═══════════════════════════════════════════════════════════════════════════╝
...
class PerceptionOutput:
    ...
    roughness_map: np.ndarray        # NEW: terrain roughness from 2nd-order gradient
...
class PerceptionModule:
    ...
    SLOPE_WEIGHT = 1.2
    OCC_WEIGHT = 1.0
    ROUGHNESS_WEIGHT = 0.8  # NEW: weight for roughness in traversability
    ...
    def process_lidar(self, measurement) -> Optional[PerceptionOutput]:
        ...
        traversability = np.clip(
            ...
            - self.ROUGHNESS_WEIGHT * roughness,  # NEW: roughness penalty
            0.0, 1.0,
        )
        ...
        out = PerceptionOutput(
            ...
            occupancy_var=var,
            slope_map=slope,
            roughness_map=roughness,  # NEW field
...
# ╔═══════════════════════════════════════════════════════════════════════════╗
# §4  SUBGOAL PLANNER (MODIFIED - added clearance penalty + roughness cost)
# ╚═══════════════════════════════════════════════════════════════════════════╝
...
class Subgoal:
    ...
    slope_deg: float
    roughness_deg: float = 0.0  # NEW: roughness at this subgoal

    cost: float = 0.0
    clearance_penalty: float = 0.0  # NEW: penalty for clearance violations
...
class VSGPSubgoalPlanner:
    def __init__(self, cfg: Config = CFG):
        ...
        self.max_roughness_thresh = 0.25  # NEW: threshold for roughness
        self.min_dist = 3.0
    ...
    def plan(
        ...
        roughness = perc.roughness_map  # NEW
        ...
        for a in alpha_range:
            ...
            for d in dist_samples:
                ...
                r_val = roughness[b_idx, a_idx]  # NEW: get roughness

                is_free = o_val < self.max_occ_thresh
                is_stable = v_val < self.max_var_thresh
                is_safe = s_val < self.max_slope_thresh
                is_smooth = r_val < self.max_roughness_thresh  # NEW roughness check

                if is_free and is_stable and is_safe and is_smooth:
                    # NEW: Approximate obstacle "height" using occupancy signal
                    # (since VSGP models distance field, not height directly)
                    height_est = max(0.0, self.cfg.subgoal_distance - d)

                    # NEW: Clearance penalty based on ground_clearance
                    clearance_penalty = max(
                        0.0,
                        height_est - self.cfg.ground_clearance
                    )

                    # NEW: Full cost function with roughness and clearance
                    # cost = (
                    #     # Direction (kd * alpha)
                    #     2.0 * abs(a) / (np.pi / 2)

                    #     # Distance reward (kD * D) - negative to reward going further
                    #     - 1.5 * d

                    #     # Steepness (ks * slope)
                    #     + 2.0 * s_val

                    #     # Sudden gradient change (roughness)
                    #     + 2.5 * r_val
                    ...
                    #     # Clearance penalty
                    #     + 5.0 * clearance_penalty
                    # )

                    # ─────────────────────────────────────────────
                    # NORMALIZED TERMS
                    # ─────────────────────────────────────────────
                    dir_term = abs(a) / (np.pi / 2)                      # α
                    dist_term = d / self.max_dist                        # D
                    slope_term = s_val                                   # slope
                    rough_term = r_val                                   # Δs
                    uncertainty_term = v_val

                    # NEW: control effort proxy (curvature-based)
                    curvature = abs(a) / (d + 1e-3)
                    control_effort = curvature                           # ≈ angular velocity proxy

                    # NEW: tracking error (e = xd - xi)
                    # here: desired direction = 0 (straight forward bias)
                    tracking_error = abs(a)

                    # NEW: clearance penalty (more realistic)
                    clearance_violation = max(
                        0.0,
                        (self.cfg.ground_clearance + 0.1) - (d * (1 - o_val))
                    )
                    ...
                    # ─────────────────────────────────────────────
                    cost = (
                        self.cfg.w_direction * dir_term
                        - self.cfg.w_distance * dist_term

                        + self.cfg.w_steepness * slope_term
                        + 1.5 * rough_term

                        + self.cfg.w_collision * uncertainty_term

                        + 2.0 * control_effort              # k_u term
                        + 1.0 * tracking_error              # e term

                        + 8.0 * clearance_violation         # stronger penalty
                    )

                    if cost < best_point_cost:
                        best_point_cost = cost
                        best_point_dist = d
                        best_clearance_penalty = clearance_penalty
                else:
                    break
            ...
            if best_point_dist > 0:
                ...
                sg = Subgoal(
                    ...
                    roughness_deg=r_val * 45.0,  # NEW: roughness in degrees
                    safe=True,
                    width_m=self.cfg.vehicle_width,
                    cost=best_point_cost,
                    clearance_penalty=best_clearance_penalty,
                )
...
# ╔═══════════════════════════════════════════════════════════════════════════╗
# §5  CONTROLLER MODULE (MODIFIED - predictive lookahead + Δu penalty)
# ╚═══════════════════════════════════════════════════════════════════════════╝
...
class Controller:
    ...
    def compute(
        ...
        # NEW: Predictive lookahead-based speed planning
        lookahead = subgoal.distance

        # Heading error
        yaw_error = alpha

        # NEW: Speed planning based on terrain + curvature
        curvature = abs(alpha) / (lookahead + 1e-3)

        vx_desired = self.cfg.base_speed * (
            1.0
            - 0.5 * curvature
            - 0.5 * (terrain_slope_deg / self.cfg.max_slope_deg)
        )

        # Ensure minimum speed for progress
        vx_desired = max(0.5, vx_desired)
    ...
    def _apply_smooth(
        ...
        # NEW: MPC-like Δu penalty - penalize sudden changes
...
class VisualizationModule:
    def __init__(self, cfg: Config = CFG):
        ...
        self._cost_rough: deque   = deque(maxlen=maxlen)  # NEW: roughness cost
        self._cost_clear: deque   = deque(maxlen=maxlen)  # NEW: clearance penalty
        ...
        *,
        ...
        with self._data_lock:
            ...
            if subgoal is not None:
                ...
                # NEW: roughness and clearance costs
                self._cost_rough.append(subgoal.roughness_deg / 45.0 * CFG.w_steepness)
                self._cost_clear.append(subgoal.clearance_penalty * 5.0)
                ...
                self._cost_rough.append(0)
                self._cost_clear.append(0)
    ...
    def _draw_cost(self, ax):
        ...
        # NEW: plot roughness and clearance costs
        ax.plot(xs, list(self._cost_rough), c="#a29bfe", lw=0.8, label="Roughness")
        ax.plot(xs, list(self._cost_clear), c="#fd79a8", lw=0.8, label="Clearance")
