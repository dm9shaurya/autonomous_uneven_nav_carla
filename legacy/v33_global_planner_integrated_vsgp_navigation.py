...
WAYPOINTS: List[Tuple[float, float]] = [
    ...
    (829.07, -391.74),   # Waypoint 4
    (723.75, -230.48),   # Waypoint 5
    (553.87, -250.54),   # Final goal  ← keep this last
]
...
class Config:
    ...
    subgoal_num_angles: int = 5
    subgoal_num_depth: int = 5
    ...
    terrain_memory_lidar_downsample: int = 50   # use every Nth LiDAR point
    terrain_memory_cost_weight: float = 4.0     # weight of memory cost in subgoal scoring
    terrain_memory_lookahead_steps: int = 3     # look-ahead horizon for candidate scoring
...
class PerceptionModule:
    ...
    def process_lidar(self, measurement) -> Optional[PerceptionOutput]:
        ...
        if len(forward_pts) < 10:
            ...
            jump_risk   = np.clip(np.max(np.abs(np.diff(fwd_ranges))) / 8.0, 0.0, 1.0) \
                          if len(fwd_ranges) > 1 else 0.0
        ...
        out = PerceptionOutput(
            ...
            traversability=traversability, raw_points=pts,
            free_mask=traversability > 0.5, mean_surface=mean,
...
class TerrainAnalyzer:
    ...
    def local_risk(self, lx: float, ly: float, radius: float = 4.5) -> Tuple[float, float, float, float, float]:
        ...
        flatness = clamp(
            1.0 - 0.55 * slope - 0.25 * rough - 0.20 * obs - 0.45 * clearance_risk,
            0.0, 1.0,
        )
...
# ╔════════════════════════════════════════════════════════════════════════════╗
# §6.5  TERRAIN MEMORY  ←  NEW MODULE
# ╚════════════════════════════════════════════════════════════════════════════╝

class TerrainMemory:
    """
    Persistent, decaying grid-based terrain cost map.

    Uses a fixed-size 2D grid indexed by quantised world coordinates.
    Each cell stores an exponentially-weighted moving average of observed
    traversal cost.  Cost accumulates from LiDAR roughness every frame and
    spikes to 1.0 at confirmed stuck positions.  The map decays uniformly
    each tick so that old obstacles gradually fade.

    Thread-safety note: all operations are single-threaded (called from the
    main tick loop), so no locking is required.
    """

    # Internal EMA blend factor for incremental cost updates
    _EMA_ALPHA = 0.15
    # Decay applied once per tick to the whole grid
    _TICK_DECAY = 0.995

    def __init__(self, size: int = 600, resolution: float = 1.0):
        """
        Args:
            size:       Grid side length in cells.  Total memory ≈ size² × 8 bytes.
            resolution: Metres per cell.  1.0 → 1 m/cell, 0.5 → 0.5 m/cell.
        """
        self.res  = float(resolution)
        self.size = int(size)
        # Cost grid: [0, 1] float, higher = worse terrain
        self.grid = np.zeros((self.size, self.size), dtype=np.float32)
        # Confidence grid: cumulative hit count (used for diagnostics / future use)
        self.conf = np.zeros((self.size, self.size), dtype=np.float32)

    # ── coordinate helpers ────────────────────────────────────────────────

    def world_to_grid(self, x: float, y: float) -> Tuple[int, int]:
        """Convert world (x, y) in metres to grid (col, row) indices."""
    ...
    def update(self, wx: float, wy: float, cost: float) -> None:
        """
        Blend `cost` into the cell at world position (wx, wy).
        Safe to call with out-of-bounds coordinates (silently ignored).
        """
        gx, gy = self.world_to_grid(wx, wy)
        if self._in_bounds(gx, gy):
            self.grid[gx, gy] = (
                (1.0 - self._EMA_ALPHA) * self.grid[gx, gy]
                + self._EMA_ALPHA * float(cost)
            )
            self.conf[gx, gy] += 1.0

    def mark_stuck(self, wx: float, wy: float, cost: float = 1.0) -> None:
        """
        Hard-set the cell to `cost` (default 1.0) regardless of EMA.
        Use when the vehicle is confirmed stuck at (wx, wy).
        """
        gx, gy = self.world_to_grid(wx, wy)
        if self._in_bounds(gx, gy):
    ...
    def get_cost(self, wx: float, wy: float) -> float:
        """
        Return the terrain cost at world position (wx, wy).
        Returns 0.0 for out-of-bounds queries.
        """
        gx, gy = self.world_to_grid(wx, wy)
        if self._in_bounds(gx, gy):
    ...
    def lookahead_cost(
        ...
        """
        Accumulate terrain cost along a straight-line look-ahead from
        (wx, wy) in direction `heading_rad` for `steps` cells.

        Args:
            wx, wy:      World start position (typically the candidate subgoal).
            heading_rad: Direction of travel in radians.
            steps:       Number of look-ahead steps.
            step_m:      Step size in metres.
            weight:      Per-step cost multiplier.

        Returns:
            Summed weighted cost along the look-ahead path.
        """
        total = 0.0
        cos_h = math.cos(heading_rad)
        sin_h = math.sin(heading_rad)
        for t in range(1, steps + 1):
            fx = wx + t * step_m * cos_h
            fy = wy + t * step_m * sin_h
    ...
    def decay(self) -> None:
        """
        Apply per-tick global decay.  Call once at the end of every control
        loop iteration so that stale observations fade over time.
        """
    ...
    def update_from_lidar(
        ...
        """
        Batch-update terrain memory from a LiDAR point cloud.

        Points are in the vehicle's local frame (x-forward, y-left, z-up).
        Cost for each point is derived from its z-deviation (roughness proxy).

        Args:
            pts:         (N, 3) array of local-frame LiDAR points.
            vehicle_x:   Vehicle world-frame X position.
            vehicle_y:   Vehicle world-frame Y position.
            downsample:  Use every `downsample`-th point to keep CPU load low.
        """
        if pts is None or len(pts) < downsample:
            return
        sampled = pts[::downsample]
        # World positions of sampled points (z ignored for grid, used for cost)
        world_x = vehicle_x + sampled[:, 0]
        world_y = vehicle_y + sampled[:, 1]
        # Roughness proxy: absolute z offset scaled to [0, 1]
        rough_cost = np.clip(np.abs(sampled[:, 2]) * 0.10, 0.0, 1.0)

        for i in range(len(sampled)):
            self.update(float(world_x[i]), float(world_y[i]), float(rough_cost[i]))
...
class VSGPSubgoalPlanner:
    ...
    def plan(
        ...
        goal_pos: np.ndarray,
        terrain_memory: Optional['TerrainMemory'] = None,   # ← NEW param
        ...
        for a in alpha_range:
            ...
            # ── TerrainMemory contribution ─────────────────────────────────
            # 1. Direct memory cost at candidate position
            # 2. Look-ahead cost along the direction of travel from the candidate
            memory_cost = 0.0
            lookahead_cost = 0.0
            if terrain_memory is not None:
                memory_cost = terrain_memory.get_cost(wx, wy)

                # heading from vehicle toward candidate
                cand_heading = math.atan2(wy - vehicle_pos[1], wx - vehicle_pos[0])
                lookahead_cost = terrain_memory.lookahead_cost(
                    wx=wx, wy=wy,
                    heading_rad=cand_heading,
                    steps=self.cfg.terrain_memory_lookahead_steps,
                    step_m=1.0,
                    weight=self.cfg.terrain_memory_lookahead_weight,
                )
            # ──────────────────────────────────────────────────────────────
            ...
            cost = (
                ...
                + self.cfg.terrain_memory_cost_weight * memory_cost    # ← memory at candidate
                + lookahead_cost                                        # ← forward look-ahead
            )
...
class MPPIPlanner:
    ...
    def _rollout(self, state, u, occ, slope, rough, var, trav, curv, void_map):
        ...
        for t in range(T):
            ...
            occ_v   = occ[bi, ai];   slope_v = slope[bi, ai]
            rough_v = rough[bi, ai]; var_v   = var[bi, ai]
            trav_v  = trav[bi, ai];  curv_v  = curv[bi, ai]
            ...
            danger = (
                (curv_v > self.cfg.cliff_curv_threshold) |
                (void_v > self.cfg.cliff_void_threshold)
            ).float()
            cost += danger * 2000.0
            ...
            if self.mem_pts is not None:
                mx  = self.mem_pts[:, 0];  my = self.mem_pts[:, 1];  mw = self.mem_pts[:, 2]
...
class Controller:
    ...
    def apply_safety_filters(
        ...
        # th *= ts
        ts = max(0.25, ts)  #never allow full zero 
...
class StuckRecovery:
    ...
    # Number of escape angle candidates to test against terrain memory
    _N_ESCAPE_ANGLES = 9
    ...
    def __init__(
        ...
        terrain_memory: Optional['TerrainMemory'] = None,    # ← NEW
    ):
        self.cfg            = cfg
        self.planner        = planner
        self.terrain_memory = terrain_memory                 # ← NEW
        self.mode           = self.IDLE
        self.mode_start     = 0.0
    ...
    def _start(self, t: float, st: VehicleState, goal_xy: Tuple[float, float]) -> None:
        ...
        # ── Mark stuck cell as bad in terrain memory ──────────────────────
        if self.terrain_memory is not None:
            self.terrain_memory.mark_stuck(st.x, st.y, cost=1.0)
            print(f"[TERRAIN_MEM] Marked stuck position ({st.x:.1f},{st.y:.1f}) cost=1.0")

        # ── Terrain-memory-aware escape angle selection ───────────────────
        # Test a fan of candidate escape angles and pick the one with the
        # lowest accumulated terrain cost.  Falls back to goal-heading bias
        # when terrain memory is unavailable or all costs are equal.
        goal_heading = math.atan2(goal_xy[1] - st.y, goal_xy[0] - st.x)
        rel          = angle_wrap(goal_heading - st.yaw)

        if self.terrain_memory is not None:
            best_angle   = rel          # default: toward goal
            lowest_cost  = float("inf")
            probe_dist   = 5.0          # metres ahead for probing

            for angle in np.linspace(-1.2, 1.2, self._N_ESCAPE_ANGLES):
                probe_x = st.x + probe_dist * math.cos(st.yaw + angle)
                probe_y = st.y + probe_dist * math.sin(st.yaw + angle)
                c = self.terrain_memory.get_cost(probe_x, probe_y)
                if c < lowest_cost:
                    lowest_cost = c
                    best_angle  = angle

            # If the best escape is essentially straight (|angle| < 0.1),
            # fall back to a goal-biased steer to avoid driving into the
            # same obstacle the vehicle just got stuck on.
            ...
            print(f"[RECOVERY] Terrain-aware escape angle={math.degrees(best_angle):.1f}°  cost={lowest_cost:.3f}")
        else:
            # Original fallback: steer away from goal heading
    ...
    def cmd(self) -> Optional[ControlState]:
        ...
        if self.mode == self.TURN:
            return ControlState(
                throttle=0.65, steer=self.recovery_steer, brake=0.0, reverse=False,
            )
...
class ResultLogger:
    ...
    def log(self, t, st, cmd, cand, goal_dist, mode, cost, reward,
            wp_idx: int = 0, total_wps: int = 1, behavior_state: str = "AUTO") -> None:
        ...
        self.rows.append({
            ...
            "mode": mode, "behavior_state": behavior_state,
...
class VisualizationModule:
        ...
        if self._lidar_pts is not None and len(self._lidar_pts):
            ...
            for i in range(0, len(pts), 3):
                ...
                if x <= px < x + w and content_y <= py < content_y + content_h:
                    t = float(zn[i])
                    if   t < 0.2: r, g, b = 255, int(255 * (t / 0.2)), 0
                    elif t < 0.4: r, g, b = int(255 * ((0.4 - t) / 0.2)), 255, 0
                    elif t < 0.6: r, g, b = 0, 255, int(255 * ((t - 0.4) / 0.2))
                    elif t < 0.8: r, g, b = 0, int(255 * ((0.8 - t) / 0.2)), 255
                    else:         r, g, b = int(255 * ((t - 0.8) / 0.2)), 0, 255
    ...
    def _draw_hud(self, W: int, H: int) -> None:
        ...
        is_final = self._active_wp_idx == n_wps - 1
        ...
        lines = [
            ...
            (f"WP   : {self._active_wp_idx+1}/{n_wps}" + (" [FINAL]" if is_final else ""), C['waypoint']),
...
class MissionManager:
    ...
    def update(self, st: VehicleState, t: float) -> bool:
        ...
        if self.stall_counter >= self.cfg.mission_stall_ticks:
            if self.is_final:
                print(f"[MISSION] ⚠ Stall on final goal ({wx:.1f},{wy:.1f}) — holding")
            else:
                print(
                    f"[MISSION] ⚠ Stall at WP {self.idx+1} ({wx:.1f},{wy:.1f})"
                    f" — skipping to WP {self.idx+2}"
                )
                self.idx      += 1
                self.last_dist = float('inf')
            self.stall_counter = 0
...
class BehaviorPlanner:
    ...
    def update(self, *, mission_complete: bool, obstacle_active: bool, in_recovery: bool) -> None:
...
class SafetyMonitor:
    ...
    def check(self, st: VehicleState) -> Tuple[bool, str]:
        ...
        if st.speed < 0.01 and abs(st.roll_deg) > 45.0:
            self._stall_detected = True
            return True, "EXTREME STALL + ROLL"
        self._flip_detected = self._stall_detected = False
        return False, ""

    def get_safe_stop_command(self) -> ControlState:
        return ControlState(throttle=0.0, steer=0.0, brake=1.0, reverse=False)
...
class ControlArbitrator:
    ...
    def arbitrate(
        ...
        behavior_mode: str,
        safety_unsafe: bool,
        recovery_cmd: Optional[ControlState],
        reactive_cmd: ControlState,
        mppi_cmd: ControlState,
        manual_cmd: Optional[ControlState],
        st: VehicleState,
        goal_dist: float,
        slope: float,
        roughness: float,
        ...
        if behavior_mode == BehaviorPlanner.GOAL_REACHED:
            return ControlState(throttle=0.0, steer=0.0, brake=1.0, reverse=False), "GOAL_REACHED"
...
class NavigationSystem:
    def __init__(self, cfg: Config = CFG):
        ...
        # ── TerrainMemory (must be created before StuckRecovery) ──────────
        self._terrain_memory = TerrainMemory(
            size=cfg.terrain_memory_size,
            resolution=cfg.terrain_memory_resolution,
        )
        ...
        # StuckRecovery now receives the terrain memory reference
        self._recovery = StuckRecovery(cfg, self._flat_planner, self._terrain_memory)
    ...
    def _fixed_spawn_candidates(self) -> List[Tuple[str, carla.Transform]]:
        ...
        for dz in [0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 5.0, 8.0]:
            tf = carla.Transform(
                carla.Location(x=self.cfg.spawn_x, y=self.cfg.spawn_y, z=self.cfg.spawn_z + dz),
                carla.Rotation(
    ...
    def _tick(self) -> None:
        ...
        # ── 4b. Update TerrainMemory from LiDAR (every frame, downsampled) ─
        # Uses vehicle world position to project local-frame LiDAR points
        # back into the fixed global grid.
        if pts is not None and len(pts) > 0:
            self._terrain_memory.update_from_lidar(
                pts=pts,
                vehicle_x=st.x,
                vehicle_y=st.y,
                downsample=self.cfg.terrain_memory_lidar_downsample,
            )
        ...
        # ── 6. Safety Check ────────────────────────────────────────────────
        safety_unsafe, safety_reason = self._safety.check(st)

        # ── 7. Planning & Command Generation ──────────────────────────────
        ...
        if bmode == BehaviorPlanner.MANUAL:
            ...
            recovery_cmd = self._recovery.cmd()
            if recovery_cmd is None:
                recovery_cmd = ControlState(throttle=0.1)
            self._stuck_memory.update(np.array([st.x, st.y]), stuck=True, failure_type="stall")

        else:
            # AUTO / AVOID_OBSTACLE
            if perc is not None:
                # VSGP subgoal planning — now receives terrain_memory
                best_subgoal, all_candidates = self._subgoal_pl.plan(
                    ...
                    goal_pos=goal_arr,
                    terrain_memory=self._terrain_memory,           # ← NEW
                )
                ...
                mppi_cmd, current_cost = self._mppi.plan(
        ...
        # ── 9. TerrainMemory decay  (once per tick, tail of loop) ─────────
        # Gradual forgetting prevents the grid from accumulating stale data
        # and ensures recovered regions become traversable again over time.
        self._terrain_memory.decay()

        # ── Cost / reward bookkeeping ──────────────────────────────────────
        cost_total = (
            current_cost
            + 0.3 * final_cmd.throttle ** 2
            + 0.8 * final_cmd.steer    ** 2
            + 0.4 * final_cmd.brake    ** 2
        )
        ...
        self._logger.log(
            ...
            behavior_state=bmode,
        )

        if self._step % (self.cfg.viz_every_n_ticks * 5) == 0:
            tip_tag = f" [SAFETY:{safety_reason}]" if safety_unsafe else ""
            print(
                f"\r[NAV] t={t:6.1f}s "
                f"WP={self._mission.idx+1}/{len(self._mission.waypoints)} "
                f"dist={goal_dist:7.2f}m "
                f"spd={st.speed:5.2f} thr={final_cmd.throttle:4.2f} "
                f"steer={final_cmd.steer:+5.2f} roll={st.roll_deg:+5.1f}° "
                f"pitch={st.pitch_deg:+5.1f}°   mode={final_mode:>22s}{tip_tag}",
                end="", flush=True,
            )
        ...
        self._viz.push(
            ...
            pitch=st.pitch_deg,
            roll=st.roll_deg,
            anti_tip_active=safety_unsafe,
            anti_tip_reason=safety_reason,
        )
