...
GP_NOISE = 0.1
GP_MAX_POINTS = 50  # Sparse GP - limit training points
GP_UPDATE_FREQ = 7   # Update GP every N frames
...
K_DIR = 5  # Direction cost weight
K_DST = 0.3  # Distance/forward cost weight
K_STP = 2  # Steepness cost weight
...
# ─────────────────────────────────────────────────────────────
# UPDATED PAPER-ALIGNED NAVIGATION OVERRIDES
# ─────────────────────────────────────────────────────────────
...
def _local_to_world(vehicle_transform, x_local, y_local, z_local=0.0):
    """Transform local vehicle-frame coordinates into world-frame coordinates."""
    loc = vehicle_transform.location
    yaw = math.radians(vehicle_transform.rotation.yaw)
    cos_y = math.cos(yaw)
    sin_y = math.sin(yaw)
    world_x = loc.x + (x_local * cos_y) - (y_local * sin_y)
    world_y = loc.y + (x_local * sin_y) + (y_local * cos_y)
    world_z = loc.z + z_local
    return world_x, world_y, world_z


class VSGPPerception:
    """
    Paper-aligned local perception:
    - LiDAR pointcloud -> spherical occupancy surface
    - high-variance surface regions are treated as free / candidate navigation space
    - contiguous free alpha-spans become navigation segments
    """
    ...
    def update(self, lidar_points):
        ...
        valid_mask = (r > 0.5) & (r < ROC * 2.0) & (np.abs(beta) < math.pi / 3)
        alpha = alpha[valid_mask]
        beta = beta[valid_mask]
        r = r[valid_mask]
        ...
        if self.frame_count % GP_UPDATE_FREQ == 0 and self.use_gp and self.gp is not None:
            try:
                self.X_train = X
                self.y_train = y
                self.gp.fit(X, y)
        ...
        if self.use_gp and self.gp is not None:
            ...
            self.variance_map = np.var(y) * np.ones_like(Alpha)
            self.mean_map = np.mean(y) * np.ones_like(Alpha)

        free_segments = self._segment_free_space(self.variance_map, Alpha, Beta)
        return free_segments, self.variance_map, self.mean_map

    def _segment_free_space(self, var_map, Alpha_grid, Beta_grid):
        """
        Paper-aligned segmentation:
        - treat high-variance areas as candidate free space
        - scan along alpha spans on each beta row
        """
        segments = []
        if var_map is None:
            return segments
        ...
        free_mask = var_map > VARIANCE_THRESHOLD
        alpha_vals = Alpha_grid[0, :]
        beta_vals = Beta_grid[:, 0]

        for row_idx, beta_val in enumerate(beta_vals):
            row_free = free_mask[row_idx]
            start_idx = None

            for col_idx, is_free in enumerate(row_free):
                if is_free and start_idx is None:
                    start_idx = col_idx
                elif (not is_free) and start_idx is not None:
                    end_idx = col_idx - 1
                    if end_idx >= start_idx:
                        alpha_start = float(alpha_vals[start_idx])
                        alpha_end = float(alpha_vals[end_idx])
                        alpha_center = float((alpha_start + alpha_end) * 0.5)
                        width_m = float(max(0.0, ROC * abs(alpha_end - alpha_start)))
                        variance = float(np.mean(var_map[row_idx, start_idx:end_idx + 1]))
                        if width_m >= (ROBOT_WIDTH + ROBOT_MARGIN) * 0.75:
                            segments.append({
        ...
        segments.sort(key=lambda s: (abs(s["beta"]), -s["width"]))
        return segments


class SubgoalPlanner:
    """
    Candidate generation + cost evaluation.
    The visible cost plot keeps the paper's 3 terms plus total.
    """

    def __init__(self):
        self.subgoals = []
        self.selected_subgoal = None
        self.selected_route_history = deque(maxlen=220)
        self.cost_history = deque(maxlen=140)
        self.trajectory_history = deque(maxlen=260)
        self.control_history = deque(maxlen=140)
    ...
    def generate_subgoals(self, free_segments, vehicle_transform, vsgp_perception):
        ...
        for seg in free_segments:
            ...
            # Wider segments get more candidate subgoals, exactly the point of the paper.
            n_slots = max(1, int(math.floor(seg["width"] / max(ROBOT_WIDTH + ROBOT_MARGIN, 0.1))))
            if n_slots > 1:
                sample_alphas = np.linspace(seg["alpha_start"], seg["alpha_end"], n_slots)
            else:
                sample_alphas = np.array([seg["alpha"]])

            distance = float(np.clip(ROC + 0.5 + 0.15 * seg["width"], SUBGOAL_DISTANCE_MIN, SUBGOAL_DISTANCE_MAX))

            for alpha in sample_alphas:
                beta = float(seg["beta"])
                x_local = distance * math.cos(beta) * math.cos(alpha)
                y_local = distance * math.cos(beta) * math.sin(alpha)
                z_local = distance * math.sin(beta)

                mean_z = 0.0
                if vsgp_perception.mean_map is not None:
                    try:
                        alpha_idx = int((alpha + math.pi / 2) / math.pi * 39)
                        beta_idx = int((beta + math.pi / 6) / (math.pi / 3) * 19)
                        alpha_idx = int(_clamp(alpha_idx, 0, 39))
                        beta_idx = int(_clamp(beta_idx, 0, 19))
                        mean_z = float(vsgp_perception.mean_map[beta_idx, alpha_idx])
                    except Exception:
                        mean_z = 0.0

                world_x, world_y, world_z = _local_to_world(vehicle_transform, x_local, y_local, z_local + mean_z)

                pitch = float(_clamp(beta, -MAX_PITCH, MAX_PITCH))
                roll = 0.0
                safe = (abs(roll) < MAX_ROLL) and (abs(pitch) < MAX_PITCH) and (seg["width"] >= ROBOT_WIDTH + ROBOT_MARGIN)

                self.subgoals.append({
                    "x": float(x_local),
                    "y": float(y_local),
                    "z": float(z_local + mean_z),
    ...
    def evaluate_costs(self, vehicle_transform):
        ...
        for sg in self.subgoals:
            ...
            distance_cost = 1.0 - _clamp(sg["distance"] / SUBGOAL_DISTANCE_MAX, 0.0, 1.0)

            dz = _clamp(sg["z"], -2.0, 2.0)
            pitch = sg["pitch"]
            steepness_cost = (dz ** 2) + math.exp(_clamp(math.copysign(1.0, pitch) if pitch != 0 else 1.0, -1.0, 1.0) * dz) * abs(pitch)
            steepness_cost = min(steepness_cost, 10.0) / 10.0

            total_cost = (K_DIR * direction_cost) + (K_DST * distance_cost) + (K_STP * steepness_cost)

            sg["cost"] = float(total_cost)
            sg["cost_breakdown"] = {
                "direction": float(K_DIR * direction_cost),
                "distance": float(K_DST * distance_cost),
                "steepness": float(K_STP * steepness_cost),
                "total": float(total_cost),
            }

        best_cost = min((sg["cost"] for sg in self.subgoals if sg["safe"]), default=float("inf"))
        if math.isfinite(best_cost):
            self.cost_history.append(self._aggregate_cost_snapshot(best_cost))

    def _aggregate_cost_snapshot(self, best_cost):
        # Keep the plot stable and honest: separate term snapshots from the chosen candidate.
        best = min((sg for sg in self.subgoals if sg["safe"]), key=lambda s: s["cost"], default=None)
        if best is None:
            return {"direction": 0.0, "distance": 0.0, "steepness": 0.0, "total": best_cost}
        snap = dict(best["cost_breakdown"])
        snap["total"] = float(best_cost)
        return snap

    def select_best_subgoal(self):
        safe_subgoals = [sg for sg in self.subgoals if sg["safe"] and math.isfinite(sg["cost"])]
        if not safe_subgoals:
            self.selected_subgoal = None
            return None

        best_cost = min(sg["cost"] for sg in safe_subgoals)
        near_best = [sg for sg in safe_subgoals if abs(sg["cost"] - best_cost) < 1e-6]

        # Tie-breaker: prefer candidates that push us into unexplored space, not the same stale groove.
        if len(near_best) > 1 and self.selected_route_history:
            last_x, last_y = self.selected_route_history[-1]

            def novelty_score(sg):
                d = math.hypot(sg["world_x"] - last_x, sg["world_y"] - last_y)
                return d

            chosen = max(near_best, key=novelty_score)
        else:
            chosen = min(safe_subgoals, key=lambda s: s["cost"])

        self.selected_subgoal = chosen
        self.selected_route_history.append((chosen["world_x"], chosen["world_y"]))
        return chosen

    def add_trajectory_point(self, location):
        self.trajectory_history.append((float(location.x), float(location.y)))

    def add_control_point(self, linear_vel, angular_vel, throttle, steer):
...
class VisualizationManager:
    """
    2x3 layout:
    Row 1: front view | rainbow LiDAR | rear view
    Row 2: trajectory map + subgoal window | cost function | linear/angular phase portrait
    """
    ...
    def _init_matplotlib_plots(self):
        ...
        self.fig_phase = plt.figure(figsize=(3, 2), dpi=80)
    ...
    def update_plots(self, planner, vehicle):
        ...
        # Cost plot
        self.ax_cost.clear()
        if len(planner.cost_history) > 0:
            costs = list(planner.cost_history)
            x_vals = range(len(costs))
            dir_costs = [c.get("direction", 0.0) for c in costs]
            dst_costs = [c.get("distance", 0.0) for c in costs]
            stp_costs = [c.get("steepness", 0.0) for c in costs]
            tot_costs = [c.get("total", 0.0) for c in costs]

            self.ax_cost.plot(x_vals, tot_costs, "k-", linewidth=2, label="Total")
            self.ax_cost.plot(x_vals, dir_costs, "b--", linewidth=1, label="Direction")
            self.ax_cost.plot(x_vals, dst_costs, "g-.", linewidth=1, label="Distance")
            self.ax_cost.plot(x_vals, stp_costs, "r:", linewidth=1, label="Steepness")
            self.ax_cost.set_ylim(0, max(1.5, max(tot_costs) * 1.2 if tot_costs else 1.5))
            self.ax_cost.set_title("Cost Function", fontsize=9)
            self.ax_cost.set_xlabel("Frame")
            self.ax_cost.set_ylabel("Cost")
            self.ax_cost.grid(True, alpha=0.3)
            self.ax_cost.legend(loc="upper right", fontsize=7)
        ...
        # Trajectory map with predicted route and subgoal window
        self.ax_traj.clear()
        traj = list(planner.trajectory_history)
        pred = list(planner.selected_route_history)
        if len(traj) > 1:
            self.ax_traj.plot([p[0] for p in traj], [p[1] for p in traj], "g-", linewidth=2, label="Actual path")
        if len(pred) > 1:
            self.ax_traj.plot([p[0] for p in pred], [p[1] for p in pred], "c:", linewidth=2, label="GP predicted route")

        if planner.subgoals:
            for sg in planner.subgoals:
                if sg == planner.selected_subgoal:
                    self.ax_traj.scatter(sg["world_x"], sg["world_y"], c="yellow", marker="o", s=90, edgecolors="black", zorder=5)
                elif not sg["safe"]:
                    self.ax_traj.scatter(sg["world_x"], sg["world_y"], c="red", marker="x", s=35, zorder=4)
                else:
                    self.ax_traj.scatter(sg["world_x"], sg["world_y"], c="dodgerblue", marker=".", s=28, alpha=0.7, zorder=4)

        if vehicle is not None:
            loc = vehicle.get_location()
            self.ax_traj.scatter(loc.x, loc.y, c="white", marker="^", s=120, label="Vehicle", zorder=6)

        self.ax_traj.set_title("Trajectory Tracking + Subgoal Window", fontsize=9)
        self.ax_traj.set_xlabel("X (m)")
        self.ax_traj.set_ylabel("Y (m)")
        self.ax_traj.grid(True, alpha=0.3)
        ...
        if traj or pred:
            xs = [p[0] for p in traj] + [p[0] for p in pred]
            ys = [p[1] for p in traj] + [p[1] for p in pred]
            margin = 3.0
            self.ax_traj.set_xlim(min(xs) - margin, max(xs) + margin)
            self.ax_traj.set_ylim(min(ys) - margin, max(ys) + margin)

        self.canvas_traj.draw()
        self.traj_surface = self._canvas_to_pygame(self.canvas_traj)

        # Phase portrait: linear vs angular velocity
        self.ax_phase.clear()
        if len(planner.control_history) > 1:
            ctrl = list(planner.control_history)
            linear = [c["linear"] for c in ctrl]
            angular = [c["angular"] for c in ctrl]
            self.ax_phase.plot(linear, angular, "m-", linewidth=2)
            self.ax_phase.scatter(linear[-1], angular[-1], c="black", s=20, zorder=4)
            self.ax_phase.set_title("Linear / Angular Phase Portrait", fontsize=9)
            self.ax_phase.set_xlabel("Linear velocity (m/s)")
            self.ax_phase.set_ylabel("Angular velocity (rad/s)")
            self.ax_phase.grid(True, alpha=0.3)
        self.canvas_phase.draw()
        self.phase_surface = self._canvas_to_pygame(self.canvas_phase)

    def _render_subgoal_window(self, surface, planner):
        if not planner.subgoals:
            return

        box = pygame.Surface((surface.get_width(), surface.get_height()), pygame.SRCALPHA)
        box.fill((0, 0, 0, 0))

        panel = pygame.Rect(8, 28, max(180, surface.get_width() - 16), min(120, surface.get_height() - 34))
        pygame.draw.rect(box, (10, 10, 10, 175), panel, border_radius=10)
        pygame.draw.rect(box, (120, 120, 120, 200), panel, 1, border_radius=10)

        title = self.bold_font.render("SUBGOAL SELECTION", True, (240, 240, 240))
        box.blit(title, (panel.x + 8, panel.y + 6))

        top = sorted(planner.subgoals, key=lambda s: s["cost"])[:5]
        y = panel.y + 26
        for idx, sg in enumerate(top, 1):
            status = "OK" if sg["safe"] else "BAD"
            txt = f"{idx}. a={sg['alpha']:+.2f}  d={sg['distance']:.1f}  c={sg['cost']:.2f}  {status}"
            color = (255, 255, 180) if sg == planner.selected_subgoal else (200, 200, 200)
            box.blit(self.font.render(txt, True, color), (panel.x + 8, y))
            y += 17
            if y > panel.bottom - 18:
                break

        box.blit(self.font.render(f"route points: {len(planner.selected_route_history)}", True, (180, 220, 255)),
    ...
    def render(self, planner, vehicle, display_manager):
        ...
        if self.cost_surface is not None:
            surf = pygame.transform.scale(self.cost_surface, (slot_w, slot_h))
            off = display_manager.get_display_offset([1, 0])
            display_manager.display.blit(surf, off)
            pygame.draw.rect(display_manager.display, (60, 60, 80), (off[0], off[1], slot_w, slot_h), 1)
            display_manager.display.blit(self.bold_font.render("COST FUNCTION", True, (230, 230, 230)), (off[0] + 4, off[1] + 4))

        if self.traj_surface is not None:
            surf = pygame.transform.scale(self.traj_surface, (slot_w, slot_h))
            off = display_manager.get_display_offset([1, 1])
            display_manager.display.blit(surf, off)
            pygame.draw.rect(display_manager.display, (60, 60, 80), (off[0], off[1], slot_w, slot_h), 1)
            display_manager.display.blit(self.bold_font.render("TRAJECTORY + SUBGOALS", True, (230, 230, 230)), (off[0] + 4, off[1] + 4))

            overlay = pygame.Surface((slot_w, slot_h), pygame.SRCALPHA)
            self._render_subgoal_window(overlay, planner)
            display_manager.display.blit(overlay, off)

        if self.phase_surface is not None:
            surf = pygame.transform.scale(self.phase_surface, (slot_w, slot_h))
            off = display_manager.get_display_offset([1, 2])
            display_manager.display.blit(surf, off)
            pygame.draw.rect(display_manager.display, (60, 60, 80), (off[0], off[1], slot_w, slot_h), 1)
            display_manager.display.blit(self.bold_font.render("PHASE PORTRAIT", True, (230, 230, 230)), (off[0] + 4, off[1] + 4))


class NavigationController:
    """
    Paper-aligned control loop with:
    - safe VSGP subgoal selection
    - stuck recovery
    - manual mode support handled outside
    """
    ...
    def _filter_lidar_for_vsgp(self, pts):
        if pts is None:
            return np.empty((0, 3))
        mask = (pts[:, 0] > 0) & (pts[:, 0] < 22.0) & (pts[:, 1] > -11.0) & (pts[:, 1] < 11.0) & (pts[:, 2] > -2.2) & (pts[:, 2] < 3.2)
        filtered = pts[mask]
        if len(filtered) > GP_MAX_POINTS * 2:
    ...
    def compute_control(self, lidar_pts, depth_raw=None, vehicle=None):
        if vehicle is None:
        ...
        self.diag["subgoals"] = len(self.planner.subgoals)
        self.diag["best_cost"] = float(best_sg["cost"]) if best_sg else float("inf")
        ...
        if best_sg is not None:
            self.diag["state"] = "VSGP-NAV"
            target_steer = float(np.clip(best_sg["alpha"] / (math.pi / 3), -1.0, 1.0))

            if best_sg["distance"] < 5.0 or best_sg["variance"] > VARIANCE_THRESHOLD * 1.1:
                throttle = THROTTLE_CAUTION

            analysis = self._analyse_lidar_legacy(nav_pts)
            if analysis:
                front = analysis["front"]
                self.diag["front"] = front
                self.diag["fl"] = analysis["fl"]
                self.diag["fr"] = analysis["fr"]
        ...
        self.steer_ema = (self.steer_ema_alpha * target_steer) + ((1.0 - self.steer_ema_alpha) * self.steer_ema)
        self.throttle_ema = (self.throttle_ema_alpha * throttle) + ((1.0 - self.throttle_ema_alpha) * self.throttle_ema)

        self.diag["steer"] = self.steer_ema
        self.diag["throttle"] = self.throttle_ema

        angular_est = float(self.steer_ema * max(speed, 1.0))
        self.planner.add_control_point(speed, angular_est, self.throttle_ema, self.steer_ema)
...
def run_simulation(args, client):
    """
    New run loop:
    - top row stays camera / lidar / rear cam
    - bottom row is plot-only: trajectory, cost, phase portrait
    - manual toggle stays simple (P or M)
    """
    display_manager = None
    visualizer = None
    actors = []
    ...
    try:
        ...
        print("🚗  Running — ESC/Q: quit   R: reset   P/M: toggle manual/auto")
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key in (K_ESCAPE, K_q):
                        ...
                        nav.planner.cost_history.clear()
                        nav.planner.control_history.clear()
                        nav.planner.selected_route_history.clear()
                        nav.reverse_count = 0
                        nav.emergency_count = 0
                        print("⟳  Nav state reset")
                        ...
                        manual_steer = 0.0
                        tag = "🕹  MANUAL" if manual_mode else "🤖  VSGP-AUTO"
                        print(f"[toggle] Switched to {tag} mode")

            if manual_mode:
                keys = pygame.key.get_pressed()
                m_throttle, m_brake, m_reverse, m_steer_tgt = 0.0, 0.0, _R_FWD, 0.0
                ...
                lidar_pts = rainbow_lidar.lidar_points
                control = nav.compute_control(lidar_pts, None, vehicle)
                vehicle.apply_control(control)
                mode_tag = "🤖  VSGP-AUTO"
