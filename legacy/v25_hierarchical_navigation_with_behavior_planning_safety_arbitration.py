...
class ReactiveObstacleAvoidance:
    ...
    def process(
        ...
        if min_dist <= self.cfg.react_emergency_dist:
            ...
            ratio = (min_dist - self.cfg.react_danger_dist) / (self.cfg.react_warn_dist -self.cfg.react_danger_dist
            )
...
class ManualController:
    ...
    def tick(self, keys, roll_deg: float = 0.0, pitch_deg: float = 0.0) -> ControlState:
        ...
        if keys[pygame.K_w]:
            ...
            self._throttle = max(self._throttle - self.THROTTLE_STEP 
* 2, 0.0)
...
# ╔══════════════════════════════════════════════════════════════════╔════════════════════════════════════════════════════════════════════════════╗
# §18.7  SAFETY MONITOR (NEW LAYER)
# ╚══════════════════════════════════════════════════════════════════╚════════════════════════════════════════════════════════════════════════════╝

class SafetyMonitor:
    """
    Lightweight safety layer detecting flip/extreme stall.
    Triggers safe stop or reset hook.
    """
    def __init__(self, cfg: Config):
        self.cfg = cfg
    ...
    def check(self, st: VehicleState) -> Tuple[bool, str]:
        ...
        # 2. Extreme Stall (Speed ~ 0 for extended period handled by StuckRecovery, 
        #    but here we check for immediate physical impossibility)
...
# ╔══════════════════════════════════════════════════════════════════╔════════════════════════════════════════════════════════════════════════════╗
# §18.8  CONTROL ARBITRATOR (NEW LAYER)
# ╚══════════════════════════════════════════════════════════════════╚════════════════════════════════════════════════════════════════════════════╝

class ControlArbitrator:
    """
    SINGLE control decision function.
    Priority: Safety > Recovery > Obstacle > Normal (MPPI)
    Ensures NO mixing of control outputs.
    """
    ...
    def arbitrate(
        ...
        if behavior_mode == BehaviorPlanner.MANUAL and manual_cmd is not None:
            # In manual, we still apply reactive avoidance for safety
        ...
        # 5. Obstacle Avoidance / Normal Auto
        # In AVOID_OBSTACLE, reactive_cmd overrides mppi_cmd significantly
        # In AUTO, reactive_cmd modifies mppi_cmd slightly
        if behavior_mode == BehaviorPlanner.AVOID_OBSTACLE:
            # Strong override
            return reactive_cmd, "AVOID_OBSTACLE"
        
        # 6. Normal Auto (MPPI Base)
        # Apply reactive adjustments to MPPI output
        final_cmd = reactive_cmd # Reactive process already merged into this in NavSystem logic
        
        # Apply Nav6 Throttle Bias (Auto only)
        # This is handled in NavigationSystem before arbitration for clarity, 
        # but here we ensure the returned command is the final one.
        return final_cmd, "AUTO"


# ╔══════════════════════════════════════════════════════════════════╔════════════════════════════════════════════════════════════════════════════╗
# §19  NAVIGATION SYSTEM  (Refactored with Hierarchical Layers)
# ╚══════════════════════════════════════════════════════════════════╚════════════════════════════════════════════════════════════════════════════╝
...
class NavigationSystem:
    ...
    def _tick(self) -> None:
        ...
        if bmode == BehaviorPlanner.MANUAL:
            ...
            # AUTO / AVOID_OBSTACLE
            if perc is not None:
                # Local subgoal planning (VSGP PRIMARY, FlatFALLBACK)
                ...
                if best_subgoal is not None:
                    _, _, _, _, clearance = self._terrain.local_risk(float(best_subgoal.local_pos[0]),
                ...
                # MPPI trajectory planning ( Receives SUBGOAL, not global waypoint )
                ...
                if rec_cmd2 is not None and self._recovery.mode != StuckRecovery.IDLE:
                    ...
                    bmode = self._behavior.mode # Update local mode
                    self._controller.reset()
                    self._stuck_memory.update(np.array([st.x, st.y]), stuck=True, failure_type="stall")
                else:
                    # Apply Reactive Avoidance to MPPI output
        ...
        # ── 8. Control Arbitration (SINGLE DECISION FUNCTION) 

        final_cmd, final_mode = self._arbitrator.arbitrate(
            behavior_mode=bmode,
            safety_unsafe=safety_unsafe,
            recovery_cmd=recovery_cmd,
            reactive_cmd=mppi_cmd, # In AUTO, mppi_cmd already has reactive processed
            mppi_cmd=mppi_cmd,
        ...
        # If safety triggered, override reason
        if safety_unsafe:
        ...
        reward_total = current_reward + 8.0 * st.speed - 0.35 * goal_dist       # ── Logging 
