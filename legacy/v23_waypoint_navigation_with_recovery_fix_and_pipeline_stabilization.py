...
#
#  The tolerance for "waypoint reached" is controlled by Config.waypoint_tolerance
#  for intermediate points, and Config.goal_tolerance for the final goal.
#
WAYPOINTS: List[Tuple[float, float]] = [
    # ── Intermediate waypoints ─────────────────────────────────
    (854.0, -625.0),    # Waypoint 1  ← edit / add / remove freely
    (836.0, -497.87),    # Waypoint 2
    (842.71, -395.0),    # Waypoint 3
    # ── Final goal ─────────────────────────────────────────────
    (769.27, -287.79),  # Final goal  ← keep this last
]
...
class Config:
    ...
    # ── Goal / Waypoints
    # final goal (used as fallback if WAYPOINTS is empty)
    goal_x: float = 769.27
    goal_y: float = -287.79
    goal_tolerance: float = 5.0
    waypoint_tolerance: float = 8.0   # ← tolerance for intermediate waypoints
    ...
    anti_tip_pitch_danger: float = 25.0
...
# ╔════════════════════════════════════════════════════════════════════════════╗
# §13  STUCK RECOVERY STATE MACHINE  (BUG FIXES APPLIED)
# ╚════════════════════════════════════════════════════════════════════════════╝
...
class StuckRecovery:
    ...
    def __init__(self, cfg: Config, planner: 'FlatGoalPlanner'):
        ...
        self.mode   = self.IDLE
        self.mode_start = 0.0
        # History stores (sim_time, x, y, speed) — maxlen covers ~30 s at 20 Hz
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # FIX A: update() now receives the ACTUAL ctrl_state (not a zeroed default).
    #         The old code called this before ctrl_state was computed, always
    #         passing throttle=0 which triggered the early-return guard every
    #         tick, making stuck detection permanently inactive.
    #
    # FIX B: The throttle/brake guard is replaced with a reliable "is the planner
    #         trying to move?" check (vehicle speed + displacement only).  Checking
    #         commanded throttle was never sound here; a vehicle can be stuck with
    #         full throttle applied.
    #
    # FIX C: The deque history search no longer breaks on the first qualifying
    #         entry (which was the OLDEST entry, potentially 30 s stale).  We now
    #         iterate the full deque and keep overwriting `old` so we end up with
    #         the MOST RECENT entry that is at least stuck_time_s old — giving
    #         the displacement reading closest to exactly that window ago.
    # ──────────────────────────────────────────────────────────────────────────
    def update(
        self,
        t: float,
        ...
        # ── If already in a recovery phase, just manage state transitions ──
        ...
        # ── FIX B: Only skip detection when deliberately braking / reversing.
        #    Do NOT skip based on low throttle — the vehicle can be stuck with
        #    full throttle (wheel-spin, terrain lock, etc.).
        if cmd.reverse or cmd.brake > 0.25:
            self.stuck_start = None
            return

        # ── FIX C: Find the most recent history entry that is >= stuck_time_s old ──
        ...
        if disp < self.cfg.stuck_disp_thresh:
            # Vehicle barely moved over stuck_time_s window
...
class NavigationSystem:
    def __init__(self, cfg: Config = CFG):
        ...
        if WAYPOINTS:
            ...
            # fallback: use single goal from Config
            self._waypoints = [(cfg.goal_x, cfg.goal_y)]

        self._wp_idx  = 0
        self._goal    = np.array(self._waypoints[0], dtype=np.float32)
        self._wp_reached_times: List[float] = []   # log when each WP was reached
    ...
    def _print_route(self) -> None:
        ...
        for i, (wx, wy) in enumerate(self._waypoints):
            tag = " ← FINAL GOAL" if i == len(self._waypoints) - 1 else ""
            print(f"  WP {i+1:2d}: ({wx:.1f}, {wy:.1f}){tag}")
        print(f"[ROUTE] {len(self._waypoints)} waypoints total\n")

    # ──────────────────────────────────────────────────────────────────────────
    # Active goal helpers
    # ──────────────────────────────────────────────────────────────────────────
    @property
    def _active_goal_xy(self) -> Tuple[float, float]:
        return (float(self._goal[0]), float(self._goal[1]))

    def _is_final_waypoint(self) -> bool:
        return self._wp_idx == len(self._waypoints) - 1

    def _tolerance_for_current_wp(self) -> float:
        """Use tighter tolerance for the final goal."""
        return self.cfg.goal_tolerance if self._is_final_waypoint() else self.cfg.waypoint_tolerance

    def _try_advance_waypoint(self, st: VehicleState, t: float) -> bool:
        """
        Check if we've reached the current waypoint.
        Returns True if the FINAL goal was just reached.
        """
        goal_dist = math.hypot(self._goal[0] - st.x, self._goal[1] - st.y)
        tol = self._tolerance_for_current_wp()

        if goal_dist > tol:
            return False

        if self._is_final_waypoint():
            print(f"\n[NAV] ✓ Final goal reached!  t={t:.1f}s")
            return True

        # Intermediate waypoint reached
        print(
            f"\n[NAV] ✓ Waypoint {self._wp_idx + 1}/{len(self._waypoints)} reached"
            f"  ({self._goal[0]:.1f}, {self._goal[1]:.1f})  t={t:.1f}s"
            f"  → advancing to WP {self._wp_idx + 2}"
        )
        self._wp_reached_times.append(t)
        self._wp_idx += 1
        self._goal = np.array(self._waypoints[self._wp_idx], dtype=np.float32)

        # Inform FlatGoalPlanner so its cached .best is invalidated
        self._flat_planner.best = None

        print(
            f"[NAV]   New active goal: ({self._goal[0]:.1f}, {self._goal[1]:.1f})"
            f"  dist={math.hypot(self._goal[0]-st.x, self._goal[1]-st.y):.1f}m"
        )
        return False
    ...
    def spawn_vehicle(self) -> None:
        ...
        print(f"[GOAL]  active WP={self._wp_idx+1}  "
              f"x={self._goal[0]:.2f} y={self._goal[1]:.2f}")
    ...
    def _apply_nav6_throttle_bias(
        ...
        if goal_dist > self._tolerance_for_current_wp() and st.speed < 0.5:
    ...
    def _tick(self) -> None:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    self._recovery.force(t, self.get_state(), self._active_goal_xy)
        ...
        # ── Waypoint / goal check ─────────────────────────────────────────────
        if self._try_advance_waypoint(st, t):
            # Final goal reached
            self.apply_control(ControlState(0.0, 0.0, 1.0, False))
            self._running = False
            return
        # Recompute goal_dist in case waypoint just advanced
        goal_dist = math.hypot(self._goal[0] - st.x, self._goal[1] - st.y)
        ...
        if self._mode == "MANUAL":
            ...
            # ── AUTO ──────────────────────────────────────────────────────────
            #
            # STEP 1: Check if we are already mid-recovery; if so use that command.
            #         (recovery.update() is called AFTER we compute ctrl_state
            #          so the detector gets the actual throttle — FIX A)
            #
            rec_cmd  = self._recovery.cmd()
            mode_str = self._recovery.mode

            if rec_cmd is not None:
                # Already in a recovery phase — use the recovery command directly
                ctrl_state = rec_cmd
                self._controller.reset()

            elif perc is not None:
                # STEP 2: VSGP subgoal planner
                ...
                if best_subgoal is not None:
                    slope_sg, rough_sg, obs_sg, flat_sg, clearance = self._terrain.local_risk(
                    ...
                    if clearance > self.cfg.clearance_block_threshold:
                        best_subgoal = None   # reject dangerous terrain

                if best_subgoal is None:
                    # STEP 3a: Fallback — flat planner
                    cand = self._flat_planner.plan(st, self._active_goal_xy)
                    ...
                    ctrl_state = self._controller.apply_safety_filters(
                        ctrl_state, st.roll_deg, st.pitch_deg, speed=st.speed
                    )
                    print(f"[FLAT-PLAN] progress={cand.progress:.2f}m flat={cand.flatness:.2f}")

                else:
                    # STEP 3b: VSGP subgoal path
                    ...
                    print(
                        f"[SUBGOAL] alpha={best_subgoal.alpha:.2f} "
            ...
            # ── FIX A: Update stuck detector with the ACTUAL ctrl_state ──────
            # This is the key fix: previously this call happened BEFORE ctrl_state
            # was computed, always receiving ControlState(throttle=0) which made
            # the throttle < 0.35 guard fire every tick, permanently resetting
            # stuck_start and preventing any recovery from triggering.
            if ctrl_state is None:
                ctrl_state = ControlState(throttle=0.1)   # fallback: creep forward

            self._recovery.update(t, st, ctrl_state, self._active_goal_xy)

            # Re-check: update() might have just triggered a new recovery
            rec_cmd2 = self._recovery.cmd()
            if rec_cmd2 is not None and self._recovery.mode != StuckRecovery.IDLE:
                # Recovery was just triggered this tick
                ctrl_state = rec_cmd2
                mode_str   = self._recovery.mode
                self._controller.reset()
        ...
        # ── Anti-tip governor (last safety pass) ──────────────────────────────
        ...
        self._logger.log(
            t, st, ctrl_state, log_cand, goal_dist, mode_str, cost_total, reward_total,
            wp_idx=self._wp_idx, total_wps=len(self._waypoints),
        )

        if self._step % (self.cfg.viz_every_n_ticks * 5) == 0:
            tip_tag = f" [ANTI-TIP:{tip_reason}]" if tip_active else ""
            print(
                f"\r[NAV] t={t:6.1f}s WP={self._wp_idx+1}/{len(self._waypoints)} "
