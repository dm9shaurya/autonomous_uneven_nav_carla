...
# -*- coding: utf-8 -*-
"""
CARLA Cybertruck Flat/Hill Goal Navigation — LOADED-MAP SAFE VERSION
====================================================================
...
Outputs are saved in: carla_loaded_map_results/
- navigation_log.csv
- controls.csv
- trajectory.csv
- cost_reward.csv
...
Run:
    python3 nav6_loaded_map_fixed.py
...
class Config:
    ...
    # If True, the script first tries your original fixed transform.
    # If it is invalid for the loaded map, it automatically falls back to a CARLA spawn point.
    use_fixed_spawn_first: bool = True

    # If True and the fixed goal is too far/invalid for the selected spawn map,
    # the script chooses a reachable-looking goal from CARLA spawn points.
    ...
    print_every: int = 5
    viz_every: int = 2
    out_dir: str = "carla_loaded_map_results"
...
class NavigationSystem:
    ...
    def connect(self):
        ...
        # Use ONLY the world already loaded in the CARLA server.
        # Do NOT call client.load_world(...).
        # Do NOT call world.get_map(), because your loaded map/OpenDRIVE fails there.
        ...
        for _ in range(5):
            self.world.tick() if self.cfg.synchronous else self.world.wait_for_tick()
    ...
    def _fixed_spawn_candidates(self) -> List[Tuple[str, carla.Transform]]:
        """Return only user-defined transforms; never use OpenDRIVE spawn points."""
    ...
    def spawn_vehicle(self):
        ...
        if self.vehicle is None:
            raise RuntimeError(
                ...
                "This script cannot use map spawn points because the loaded map OpenDRIVE cannot be parsed. "
                "Change Config.spawn_x/spawn_y/spawn_z/spawn_yaw to a free position in your loaded map. "
                f"Last CARLA error: {last_error}"
            )

        # Do NOT call vehicle.set_autopilot(False). Even False can trigger TrafficManager/map
        # generation in some CARLA builds, which fails when OpenDRIVE is invalid.
        self.vehicle.set_simulate_physics(True)
        self.vehicle.apply_control(carla.VehicleControl(throttle=0.0, brake=1.0, hand_brake=False, manual_gear_shift=False))
        self._safe_world_tick(int(self.cfg.post_spawn_wait_s / self.cfg.dt))
