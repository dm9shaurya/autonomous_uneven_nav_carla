...
class Config:
    ...
    anti_tip_roll_warn: float = 6.0       # deg – start throttle reduction
    anti_tip_roll_danger: float = 11.0    # deg – hard-brake + corrective steer
    anti_tip_pitch_warn: float = 8.0      # deg – start throttle reduction
    anti_tip_pitch_danger: float = 16.0   # deg – throttle cut + brake
    anti_tip_throttle_floor: float = 0.25 # minimum throttle while cornering steeply
    ...
    min_steer_at_speed: float = 0.18      # never clamp steer below this (need to turn)

    # ── Terrain Speed Scaling (NEW)
    max_speed_on_slope: float = 6.0       # m/s cap when slope > 0.4
    max_speed_on_rough: float = 7.5       # m/s cap when roughness > 0.5
...
class PerceptionModule:
    ...
    def process_lidar(self, measurement) -> Optional[PerceptionOutput]:
        ...
        #     - self.VAR_FREE_WEIGHT * var_n
        #     - self.ROUGHNESS_WEIGHT * rough_n
        #     - self.CURV_WEIGHT * curv_n,   # 👈 NEW: harsh penalty
        #     0.0, 1.0,
        # )

        CURV_WEIGHT = 3.0  # tune 2.0 → 5.0
...
class NavigationSystem:
    ...
    def _fixed_spawn_candidates(self) -> List[Tuple[str, carla.Transform]]:
        """Return user-defined transforms with multiple z-offsets"""
    ...
    def spawn_vehicle(self) -> None:
        ...
        self._vehicle.apply_control(
            carla.VehicleControl(throttle=0.0, brake=1.0, hand_brake=False, manual_gear_shift=False)
        )
        self._safe_world_tick(int(self.cfg.post_spawn_wait_s / self.cfg.fixed_delta_seconds))
        self._vehicle.apply_control(
            carla.VehicleControl(throttle=0.0, brake=0.0, hand_brake=False, manual_gear_shift=False)
        )
    ...
    def _tick(self) -> None:
        ...
        if self._mode == "MANUAL":
            ...
            if rec_cmd is not None:
                ...
                if best_subgoal is None:
                    ...
                    ctrl_state, current_cost = self._mppi.plan(state_vec, perc, subgoal_for_viz, mem_tensor)
                    ctrl_state = self._controller.apply_safety_filters(ctrl_state, st.roll_deg, st.pitch_deg)
                    ...
                    state_vec = [st.x, st.y, st.yaw, st.speed]
                    mem_tensor = self._stuck_memory.get_tensor(self._mppi.device)
                    ctrl_state, current_cost = self._mppi.plan(state_vec, perc, subgoal_world, mem_tensor)
                    ctrl_state = self._controller.apply_safety_filters(ctrl_state, st.roll_deg, st.pitch_deg)

                    print(f"[SUBGOAL] alpha={best_subgoal.alpha:.2f} dist={best_subgoal.distance:.1f}m "
                          f"progress={best_subgoal.goal_progress:.2f}m slope={best_subgoal.slope:.2f} "
                          f"rough={best_subgoal.roughness:.2f} cost={best_subgoal.cost:.2f}")
        ...
        if all_candidates and all_candidates[0] is not None:
            ...
            cand = Candidate(st.x, st.y, 0, 0, 0, 0, current_progress, goal_dist, 0,
                        current_flatness, 0, 0, current_flatness, 0, current_cost, current_reward)
