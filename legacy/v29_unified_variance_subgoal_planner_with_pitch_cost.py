...
class MecanumKinematics:
    ...
    def forward(self, wheel_omegas: np.ndarray) -> Tuple[float, float, 
float]:
    ...
    def inverse(self, vx: float, vy: float, omega: float) -> np.ndarray:
        ...
        ws = self.Jinv @ twist
        ws = np.clip(ws, -self.cfg.max_wheel_omega, 
self.cfg.max_wheel_omega)
        return ws
    ...
    def to_carla_control(
        ...
        if vx < -0.1:                   # reversing
            throttle = np.clip(-vx / cfg.base_speed, 0.0, 
cfg.max_throttle)
            brake    = 0.0
...
class VSGP:
    def __init__(self, cfg: Config = CFG):
        ...
        a_vals = np.linspace(-np.pi / 2, np.pi / 2, 
int(math.sqrt(self.n_ind)))
        b_vals = np.linspace(-np.pi / 4, np.pi / 4, 
int(math.sqrt(self.n_ind)))
...
class SubgoalPlanner:
    ...
    Replaces old segment extraction + separate costing.
    """
    ...
    def plan(self, perc: PerceptionOutput, vehicle_pitch_rad: float) -> Tuple[Optional[Subgoal], List[Subgoal]]:
        """
        ...
        # Threshold for free space based on variance (high variance = unknown/free)
        threshold = np.percentile(var_row, 60)
        mask = var_row > threshold
        ...
        for i in range(len(mask)):
            if mask[i] and not in_seg:
                ...
                β_s = np.mean(beta_row[start:end+1])
                z_s = self.cfg.subgoal_distance * np.sin(β_s)

                # ── feasibility check ────────────────────────────
                req_w = self.cfg.robot_width + self.cfg.safety_margin
                slope_deg = abs(np.degrees(β_s))

                # If segment is too narrow or steep, skip
                if width_m <= req_w or slope_deg > self.cfg.max_slope_deg:
                    continue

                # ── number of subgoals (paper logic) ─────────────
                n = int(width_m // req_w)
                n = max(1, min(n, 5))

                alphas = np.linspace(α_sb, α_se, n)

                # ── evaluate each candidate immediately ──────────
                for a in alphas:
                    ...
                    # ── COST COMPUTATION (merged Step C) ─────────
                    dz = lz

                    # steepness cost (elevation + pitch interaction)
                    elev_cost  = dz ** 2
                    # Ensure pitch is reasonable for exp function
                    pitch_cost = np.exp(np.sign(vehicle_pitch_rad) * dz * 0.5) * abs(vehicle_pitch_rad)
                    C_stp = elev_cost + pitch_cost

                    # direction cost
                    C_dir = abs(a) / (np.pi / 2)

                    # forward exploration cost
                    C_dst = 1.0 - (d / self.cfg.subgoal_distance)

                    # oscillation penalty (from history)
                    osc_penalty = 0.0
                    if len(self._alpha_history) >= 2:
                        signs = [math.copysign(1, val) for val in self._alpha_history]
                        alternations = sum(1 for k in range(1, len(signs)) if signs[k] != signs[k-1])
                    ...
                    J = (
                        ...
                        self.cfg.w_steepness * C_stp +
                        self.cfg.w_oscillation * osc_penalty
                    )
        ...
        if best_subgoal is None:
            ...
            best_subgoal = Subgoal(
                ...
                distance=d,
                local_pos=np.array([lx, ly, lz]),
                slope_deg=abs(np.degrees(b)),
                safe=False,
                width_m=0.5,
                cost=999,
            )
            all_subgoals.append(best_subgoal)

        # Update history for oscillation check
        if best_subgoal:
            self._alpha_history.append(best_subgoal.alpha)

        return best_subgoal, all_subgoals
...
class NavigationSystem:
    ...
    def _tick(self) -> None:
        ...
        if self._mode == "MANUAL":
            ...
            # Stuck detection logic (FIX 2 preserved)
            ...
            if stuck:
                ...
                if perc is not None:
                    ...
                    best_goal, _ = self._planner.plan(perc, vehicle_pitch_rad)
