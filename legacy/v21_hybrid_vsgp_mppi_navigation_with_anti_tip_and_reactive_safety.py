...
class Config:
    ...
    # ── Anti-tip Governor (NEW) ───────────────────────────────────────────────
    # Roll thresholds
    anti_tip_roll_warn: float = 15.0       # deg – throttle starts reducing
    anti_tip_roll_danger: float = 15.0    # deg – hard brake + corrective steer
    # Pitch thresholds
    anti_tip_pitch_warn: float = 25.0      # deg – throttle starts reducing
    anti_tip_pitch_danger: float = 25.0   # deg – hard throttle cut + brake
    # Governor strength
    anti_tip_throttle_floor: float = 0.25 # min throttle at pitch danger (avoid stall)
    anti_tip_speed_scale: float = 0.30    # throttle multiplier at danger threshold

    # ── Speed-Steer Coupling (NEW) ────────────────────────────────────────────
    speed_steer_coupling: float = 0.55    # how aggressively steer is capped at speed
    min_steer_at_speed: float = 0.18      # floor – never clamp steer below this

    # ── Terrain Speed Caps (NEW) ──────────────────────────────────────────────
    max_speed_on_slope: float = 6.0       # m/s cap when forward slope > 0.4
    max_speed_on_rough: float = 7.5       # m/s cap when forward roughness > 0.5
...
# ╔════════════════════════════════════════════════════════════════════════════╗
# §3  VARIATIONAL SPARSE GAUSSIAN PROCESS
# ╚════════════════════════════════════════════════════════════════════════════╝
...
class PerceptionModule:
    ...
    def process_lidar(self, measurement) -> Optional[PerceptionOutput]:
        ...
        curv_min, curv_max = curvature.min(), curvature.max()
        if curv_max - curv_min < 1e-6:
            curv_n = np.zeros_like(curvature)
        else:
            curv_n = (curvature - curv_min) / (curv_max - curv_min)

        slope = np.sqrt(da ** 2 + db ** 2)
        roughness = np.sqrt(
            np.gradient(da, axis=1) ** 2 + np.gradient(db, axis=0) ** 2
        )
...
# ╔════════════════════════════════════════════════════════════════════════════╗
# §6  VSGP SUBGOAL PLANNER  – increased slope/roughness/terrain weights
# ╚════════════════════════════════════════════════════════════════════════════╝
...
class VSGPSubgoalPlanner:
    ...
    # Increased from original to prefer safer, flatter paths
    W_TERRAIN = 3.5   # was 1.8
    W_GOAL_DIST = 3.5
    W_HEADING = 0.9
    W_SLOPE = 2.8     # was 1.2
    W_ROUGH = 3.2     # was 1.0
...
# ╔════════════════════════════════════════════════════════════════════════════╗
# §9  MPPI PLANNER  – lateral G-force and speed-terrain risk penalties added
# ╚════════════════════════════════════════════════════════════════════════════╝
...
class MPPIPlanner:
    ...
    def _rollout(self, state, u, occ, slope, rough, var, trav):
        ...
        for t in range(T):
            ...
            # ── NEW: Lateral G-force proxy – penalise sharp steer at speed (tip risk)
            lateral_g = (steer.abs() * v.abs()) / max(self.cfg.wheel_base, 1e-3)
            cost += 12.0 * lateral_g.clamp(0.0, 3.0) ** 2

            # ── NEW: Speed-terrain risk – penalise high speed on bad terrain
...
# ╔════════════════════════════════════════════════════════════════════════════╗
# §10  REACTIVE OBSTACLE AVOIDANCE
# ╚════════════════════════════════════════════════════════════════════════════╝
...
# ╔════════════════════════════════════════════════════════════════════════════╗
# §11  CONTROLLER  – speed-steer coupling + earlier stability thresholds
# ╚════════════════════════════════════════════════════════════════════════════╝
...
class Controller:
    ...
    def apply_safety_filters(
        ...
        roll_deg: float,
        pitch_deg: float,
        speed: float = 0.0,   # NEW – used for speed-steer coupling
    ) -> ControlState:
        a = 0.3
        ...
        # ── Speed-steer coupling: clamp max steer based on current speed
    ...
    def _stability(self, roll: float, pitch: float) -> Tuple[float, float]:
        """
        Uses anti_tip warn/danger thresholds for earlier, more
        graduated intervention compared to the original hard threshold.
        """
        ts = 1.0
        cs = 0.0
        ar = abs(roll)
...
# ╔════════════════════════════════════════════════════════════════════════════╗
# §12  STUCK RECOVERY STATE MACHINE
# ╚════════════════════════════════════════════════════════════════════════════╝
...
class StuckRecovery:
    ...
    def _start(self, t: float, st: VehicleState) -> None:
        print(f"\n[RECOVERY] Vehicle stuck at ({st.x:.1f},{st.y:.1f}); reverse + push.")
        ...
        for dist in self.candidate_distances:
            for alpha in angles:
                ...
                reward = (
                    ...
                    - 25.0 * obs - 12.0 * slope - 8.0 * rough - 40.0 * mem
                )
                candidates.append(Candidate(
                    wx, wy, lx, ly, alpha, dist, progress, new_gdist,
                    heading_error, slope, rough, obs, flat, mem, cost, reward,
                ))
...
class ResultLogger:
    ...
    def log(self, t, st, cmd, cand, goal_dist, mode, cost, reward) -> None:
    ...
    def save_csv(self) -> None:
        ...
        write_subset("cost_reward.csv",
                     ["time", "cost", "reward", "goal_distance",
                      "flatness", "slope", "roughness", "progress"])
...
class VisualizationModule:
    C = {
        ...
        'memory': (255, 110, 50), 'cost': (255, 107, 107), 'speed': (85, 239, 196),
        'steer': (162, 155, 254), 'white': (230, 230, 230), 'dim': (110, 110, 110),
        'danger': (255, 50, 50), 'ok': (80, 220, 80), 'warn': (255, 200, 0),
    }
    ...
    def _draw_trajectory(self, x, y, w, h):
        ...
        pygame.draw.rect(self._screen, C['panel'], (x, y, w, h))
        pygame.draw.rect(self._screen, C['header'], (x, y, w, 18))
        self._screen.blit(self._font_s.render("Trajectory / Subgoal Map", True, C['white']),
                          (x + 4, y + 3))
...
class NavigationSystem:
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # NEW: Anti-tip Governor
    # ──────────────────────────────────────────────────────────────────────────
    def _anti_tip_governor(
        self, ctrl: ControlState, st: VehicleState, goal_dist: float
    ) -> Tuple[ControlState, bool, str]:
        """
        Hard safety layer that runs after all planners and reactive avoidance.
        Returns (modified_ctrl, was_active, reason_string).

        Three protections:
          1. Roll danger  → hard brake + corrective steer
          2. Pitch danger → throttle cut + brake
          3. Speed-steer  → clamp steer based on current speed
          4. Terrain caps → reduce throttle/brake on steep/rough ground
        """
        out = ControlState(
        ...
        # ── 2. Pitch: graduated throttle cut (handles both steep ascent and descent)
        if ap >= self.cfg.anti_tip_pitch_danger:
            # Allow minimum throttle so the vehicle doesn't stall on a climb
        ...
        # ── 3. Speed-steer coupling: hard clamp steer proportional to speed
        #       This prevents high-speed sharp turns (most common rollover cause)
        ...
        # ── 4. Terrain speed caps from live perception
    ...
    def spawn_vehicle(self) -> None:
        ...
        print(f"[GOAL]  x={self.cfg.goal_x:.2f} y={self.cfg.goal_y:.2f} "
              f"tol={self.cfg.goal_tolerance:.2f} m")
    ...
    def _tick(self) -> None:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    self._recovery.force(time.time() - self._start_wall, self.get_state())
        ...
        if self._mode == "MANUAL":
            ...
            if rec_cmd is not None:
                ...
                # Try VSGP subgoal planner first
                ...
                if best_subgoal is None:
                    ...
                    # Apply safety filters (speed passed for coupling)
        ...
        # ── Nav6 throttle bias (speed floor + terrain-aware target speed)
        ...
        # ── Anti-tip governor (LAST safety pass – overrides everything)
        ...
        # ── Force velocity push if in recovery forward phase
        ...
        if all_candidates and all_candidates[0] is not None:
            ...
            cand = Candidate(
                ...
                current_flatness, 0, 0,
                current_flatness, 0, current_cost, current_reward,
            )
        self._logger.log(t, st, ctrl_state, cand, goal_dist, mode_str, cost_total, reward_total)

        if self._step % (self.cfg.viz_every_n_ticks * 5) == 0:
            tip_tag = f" [ANTI-TIP:{tip_reason}]" if tip_active else ""
            print(
                f"\r[NAV] t={t:6.1f}s dist={goal_dist:7.2f}m "
